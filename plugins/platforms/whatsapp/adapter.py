...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def _bridge_send_timeout(chat_id: str) -> int:
        """Leave paced group requests enough time to reach the bridge's serialized sender."""
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                ...
                result = await self._post_bridge_message(
                    "send", payload, timeout=self._bridge_send_timeout(chat_id)
                )
    ...
    async def edit_message(self, chat_id: str, message_id: str, content: str, *, finalize: bool = False) -> SendResult:
        try:
            target = to_whatsapp_jid(chat_id)
            async with self._bridge_req("post", "edit", self._bridge_send_timeout(target), json={"chatId": target, "messageId": message_id, "message": content}) as resp:
    ...
    async def send_poll(self, chat_id: str, question: str, options: list[str], *, selectable_count: int = 1) -> SendResult:
        ...
        return await self._post_bridge_message(
            "send-poll", payload, timeout=self._bridge_send_timeout(payload["chatId"])
        )
    ...
    async def send_location(self, chat_id: str, latitude: float, longitude: float, *, name: Optional[str] = None, address: Optional[str] = None,
        ...
        return await self._post_bridge_message(
            "send-location", payload, timeout=self._bridge_send_timeout(payload["chatId"])
        )
...
def _apply_yaml_config(yaml_cfg: dict, whatsapp_cfg: dict) -> dict | None:
    ...
    for key, env in _YAML_SCALAR_KEYS:
        if key in whatsapp_cfg and not os.getenv(env):
            os.environ[env] = str(whatsapp_cfg[key])
    return None
