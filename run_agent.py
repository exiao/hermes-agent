...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        from gateway.session_context import get_session_source
...
class AIAgent(
    ...
    def _announce_or_buffer_fallback(self, message: str) -> None:
        """Emit fallback transitions live only when the fork option is enabled."""
    ...
    def _swap_credential(self, entry) -> None:
        """Keep credential rotation on the configured provider endpoint."""
        try:
            from agent.credential_pool import _get_configured_provider_base_url

            configured_base = _get_configured_provider_base_url(self.provider)
        except Exception:
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        from tools.delegate_tool import _model_role, _strip_model_hidden_task_fields, delegate_task as _delegate_task
        ...
        return _delegate_task(
            ...
            max_iterations=function_args.get("max_iterations"), role=_model_role(function_args.get("role")),
