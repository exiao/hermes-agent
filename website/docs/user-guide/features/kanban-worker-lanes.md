...
- `kanban_block(reason=..., kind=...)` — task waits for human input, status flips to `blocked`. The dispatcher respawns when `kanban_unblock` runs. (`kind` is required — `dependency` / `needs_input` / `capability` / `transient`.)
