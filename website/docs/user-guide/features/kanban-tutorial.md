...
- `hermes kanban notify-subscribe <task> --platform telegram --chat-id <id>` — get a passive gateway ping when a specific task finishes. Add `--chat-type` and the matching routing identity before relying on the default wake.
