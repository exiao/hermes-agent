...
# Keep the fork's factual kanban failure detail on the upstream split notification pipeline. The
# rebound collector resolves this name from the server namespace, so overriding the formatter here
# updates TUI/Desktop delivery without duplicating the collector itself.
_upstream_format_kanban_event_text = _format_kanban_event_text
...
def _format_kanban_event_text(sub: dict, task, ev, board_slug: str) -> Optional[str]:
    ...
    if kind not in {"gave_up", "crashed", "timed_out"}:
        return _upstream_format_kanban_event_text(sub, task, ev, board_slug)
    ...
    head = f"{board_tag}{tag}Kanban {task_id}"
    if kind == "gave_up":
        return (
            f"✖ {head} gave up after repeated spawn failures — {title}"
            f"{_failure_detail(payload, terminal=True)}"
        )
    if kind == "crashed":
        return f"✖ {head} worker crashed (pid gone) — {title}{_failure_detail(payload)}"
    return f"⏱ {head} timed out — {title}{_failure_detail(payload)}"


def _persist_model_switch(result) -> None:
    """Do not persist a provider-derived loopback base URL as a user override."""
    from cli import save_config_value

    save_config_value("model.default", result.new_model)
    save_config_value("model.provider", result.target_provider)
    base_url = result.base_url if not getattr(result, "base_url_from_provider_config", False) else None
