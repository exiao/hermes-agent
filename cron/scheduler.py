...
def _env_home_target_chat_id(platform_name: str) -> str:
    ...
    Fork: reads through ``get_secret`` rather than ``os.getenv`` so the value
    honors the active profile secret scope under multiplexing — ``os.environ``
    is process-global and races across concurrent per-profile cron jobs (each
    ``run_job`` mutates it via ``load_dotenv``), which would leak Profile A's
    home channel to Profile B.
    """
...
def _get_home_target_thread_id(platform_name: str) -> Optional[str]:
    ...
    if platform_name.lower() == "telegram":
        ...
        if cron_thread:
            return cron_thread
    # Fork: get_secret (not os.getenv) so the thread override honors the
    # active profile secret scope under multiplexing.
    value = (get_secret(f"{env_var}_THREAD_ID", "") or "").strip() if env_var else ""
...
def _is_channel_dm_topic(
    ...
    """Decide whether an ambiguous Telegram topic target is a channel DM topic."""
    # Resolve on the CLASS, not the instance: a MagicMock instance auto-creates a
    # truthy ``get_chat_info`` attribute, so an instance-level probe would
    # misclassify test doubles. Real adapters expose the coroutine on the class.
        ...
        if not delivered:
            ...
            try:
                ...
                if _interpreter_shutting_down(run_err):
                    ...
                    delivery_errors.extend(target_errors)
                    continue
                # The fresh-loop fallback can itself raise (SMTP ConnectionError,
                # future.result timeout, etc.). A per-target failure must be
                # logged and the delivery loop must continue. ContextVars (the
                # active profile secret scope) do NOT propagate into a worker
                # automatically, so copy the current context into the fallback.
                try:
