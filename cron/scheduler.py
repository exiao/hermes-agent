...
class InactivityBreach:
    """Idle reading that actually tripped the inactivity watchdog."""
...
def _inactivity_watchdog_loop(
    *, get_idle_seconds: Callable[[], float], limit_s: float, poll_s: float, stop: threading.Event,
    future_done: Callable[[], bool], required_breaches: int = 2,
):
    """Poll idle time until a sustained breach, stop, or future completion.
    ...
    of #94285 — the 4118s-idle-on-a-600s-limit cron hang). A breach must persist across consecutive polls,
    and the returned object carries the sampled idle value that caused the decision.
    """
    consecutive = 0
    breach_idle = 0.0
...
def _preflight_or_block(
    job: dict, job_id: str, job_name: str, cfg: dict, adapters=None,
) -> Optional[tuple]:
    ...
    try:
        if _cron_preflight_enabled(cfg):
            _pf_reason = _preflight_job_config(job, cfg, adapters=adapters)
...
def _raise_inactivity_timeout(agent, job_name: str, limit_s: float, breach=None) -> None:
    ...
    _secs_ago = getattr(breach, "idle_seconds", _activity.get("seconds_since_activity", 0))
    logger.error(
...
def _run_agent_with_watchdog(
    ...
    def _watch_inactivity() -> None:
        ...
        if _cron_inactivity_limit is None:
            return
        breach = _inactivity_watchdog_loop(
    ...
    if _inactivity_timeout:
        _raise_inactivity_timeout(agent, job_name, _cron_inactivity_limit, _inactivity_timeout)
...
def _resolve_cron_agent_setup(
    job: dict, job_id: str, job_name: str, jc, *, adapters=None,
) -> _CronAgentSetup:
    ...
    setup.blocked = _preflight_or_block(
        job, job_id, job_name, _cfg, adapters=adapters,
    )
...
def run_job(
    ...
    try:
        ...
        setup = _resolve_cron_agent_setup(
            job, job_id, job_name, jc, adapters=adapters,
        )
        if setup.blocked is not None:
...
from cron.scheduler_delivery import (  # noqa: E402
    _cron_mirror_delivery_enabled, _deliver_result, _delivery_lane_value,
