...
class _FrontedAdapterMap(dict):
    """Adapter map whose disabled entries are intentional shared transports."""

    def __init__(self, values, borrowed_platforms) -> None:
        super().__init__(values)
        self.allow_disabled_native_platforms = frozenset(borrowed_platforms)


def _fronted_tick_adapters(profile_name, own_adapters, shared_adapters, logger=None):
    """Borrow only shared adapters explicitly disabled by a secondary profile.

    ``enabled: false`` prevents the secondary from opening a duplicate connection and is the
    deliberate signal that the default profile fronts that platform. Enabled or absent platform
    blocks remain strictly isolated.
    """
    own = own_adapters or {}
    ...
    try:
        ...
        if logger is not None:
            logger.debug("cron: could not read profile '%s' config for fronted adapters: %s",
                         profile_name, exc)
        return own
    ...
    borrowed = {
        ...
        and (pconfig := platforms.get(platform)) is not None
        and getattr(pconfig, "enabled", True) is False
    }
    if not borrowed:
        return own
    return _FrontedAdapterMap({**own, **borrowed}, borrowed)
...
class InProcessCronScheduler(CronScheduler):
    ...
    def _start_multiplex(
        ...
        def tick_adapters_for(profile_name):
            ...
            fronted = _fronted_tick_adapters(profile_name, tick_adapters, adapters, logger)
            if fronted:
                return fronted
            if adapters:
