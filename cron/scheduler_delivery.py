...
def _resolve_target_transport(
    ...
    allow_disabled_native = bool(
        target_adapters and target_adapters.get(platform) is not None
    )
    ...
    if transport is not None and transport.is_relay:
        ...
        return None, (
            f"platform '{platform_name}' target '{target.get('chat_id')}' "
            "not configured/enabled"
        )
    elif adapters is not None and transport is None:
        logger.info(
            "Job '%s': platform '%s' target '%s' has no live adapter or relay available",
            job["id"], platform_name, target.get("chat_id"),
        )
...
def _live_send_text(
    ...
    router = DeliveryRouter(
        t.config,
        t.target_adapters,
        allow_disabled_native=bool(t.transport is not None and not t.transport.is_relay),
    )
...
def _delivery_secret_scope():
    """Install the active profile's secret scope for direct delivery calls."""
    ...
    if not is_multiplex_active() or current_secret_scope() is not None:
        yield
        return
    token = set_secret_scope(build_profile_secret_scope(_sched._get_hermes_home()))
    try:
        yield
    finally:
...
def _deliver_result(
    ...
    """Profile-scoped public delivery entry point."""
    with _delivery_secret_scope():
        return _deliver_result_impl(
            job, content, adapters=adapters, loop=loop, for_failure=for_failure,
        )


def _deliver_result_impl(
    job: dict, content: str, adapters=None, loop=None, *, for_failure: bool = False
) -> Optional[str]:
