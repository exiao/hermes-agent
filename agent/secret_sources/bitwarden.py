...
_BWS_RUNTIME_ENV_EXACT = frozenset({
    ...
    "http_proxy", "https_proxy", "all_proxy", "no_proxy", "SYSTEMROOT",
    "SYSTEMDRIVE", "WINDIR", "COMSPEC", "PATHEXT", "USERPROFILE", "APPDATA",
    "LOCALAPPDATA", "PROGRAMDATA", "HOMEDRIVE", "HOMEPATH",
    "NUMBER_OF_PROCESSORS", "NO_COLOR", "RUST_LOG", "RUST_BACKTRACE",
})


def _is_bws_runtime_env(name: str) -> bool:
    return name in _BWS_RUNTIME_ENV_EXACT or name.startswith("LC_")
...
def _run_bws_list(
    ...
    if scoped_env is not None:
        env = {
            ...
            if isinstance(value, str) and _is_bws_runtime_env(key)
        }
        env.pop("BWS_SERVER_URL", None)
        env.update(
            (key, value) for key, value in scoped_env.items()
