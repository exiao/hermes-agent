...
def _fetch_with_timeout(source: SecretSource, cfg: dict, home_path: Path,
                        environ: MutableMapping[str, str], *, scoped: bool = False,
                        fetch_environ: Optional[MutableMapping[str, str]] = None) -> FetchResult:
    ...
    try:
        ...
        is_builtin = _SOURCE_ORIGINS.get(source.name) == "builtin"
        if scoped and not is_builtin and "environ" not in fetch_params:
            return FetchResult().fail(
                f"secret source '{source.name}' cannot consume a scoped "
                "credential environment; skipped to prevent cross-profile leakage",
        ...
        future = executor.submit(contextvars.copy_context().run, _fetch)
        try:
