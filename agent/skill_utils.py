...
def is_excluded_skill_dir_name(name: str) -> bool:
    """Dot-directories are metadata/backups, never live skill categories."""
...
def is_excluded_skill_path(path, *, root: Optional[Path] = None) -> bool:
    ...
    if any(part in EXCLUDED_SKILL_DIRS for part in path_obj.parts):
        return True
    relative_parts: tuple[str, ...] = ()
    if root is not None:
        try:
            relative_parts = path_obj.relative_to(PurePath(str(root))).parts
        except ValueError:
            pass
...
def extract_skill_description(frontmatter: Dict[str, Any]) -> str:
    """Extract the full description; authoring validation owns the length cap."""
