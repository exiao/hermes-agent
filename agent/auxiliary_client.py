    ...
    with _client_cache_lock:
        ...
        for key in stale_keys:
            ...
            if client is not None:
                _release_cached_client_fds(client)
    ...
    with _client_cache_lock:
        for key in list(_client_cache.keys()):
            ...
            if cached is target or real is target:
                _release_cached_client_fds(cached)
...
def resolve_provider_client(
    ...
    # Record whether the CALLER explicitly supplied a model BEFORE any fallback.
    # ``provider == "auto"`` has its own runtime-aware resolver below; pre-filling
    # a blank auto request from config/main can pair a stale configured model with
    # the live fallback provider. Only a caller-supplied model should override
    # auto-detection. (# compression-current-model)
    ...
    if pconfig.auth_type == "external_process":
        ...
        # OAuth providers — route through their specific client adapters.
        if provider == "nous":
...
def _store_cached_client(cache_key: tuple, client: Any, default_model: Optional[str], *, bound_loop: Any = None) -> None:
    with _client_cache_lock:
        ...
        if old_entry is not None and old_entry[0] is not client:
            _release_cached_client_fds(old_entry[0])
...
def _release_cached_client_fds(client: Any) -> None:
    ...
    - Async SDK client: keep the CLOSED state-mark (the ``__del__`` neuter relies
      on it) AND sync-close the pool's raw sockets, since no event loop is
      guaranteed alive to run ``aclose()``.
    """
    import inspect
    ...
    with _client_cache_lock:
        if cache_key in _client_cache:
            ...
            if async_mode:
                ...
                _release_cached_client_fds(cached_client)
        ...
        with _client_cache_lock:
            if cache_key not in _client_cache:
                ...
                while len(_client_cache) >= _CLIENT_CACHE_MAX_SIZE:
                    ...
                    _release_cached_client_fds(evict_entry[0])
