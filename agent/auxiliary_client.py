    ...
    with _client_cache_lock:
        ...
        for key, entry in _client_cache.items():
            ...
            if cached_loop is not None and cached_loop.is_closed():
                ...
                stale_clients.append((client, cached_loop))
        for key in stale_keys:
            del _client_cache[key]
    for client, cached_loop in stale_clients:
        # Upstream drains the transport properly now (close_async=True runs
        # aclose on a fresh loop). The fork's fd reap stays as the backstop for
        # transports whose aclose can't run at all — a state-marked-but-open
        # socket is what accumulates to the fd ceiling and tears kanban.db.
    ...
    with _client_cache_lock:
        if cache_key in _client_cache:
            ...
            if async_mode:
                ...
                # Fork fd reap. Pass cached_loop: if it's alive (a different but
                # running loop holds this cache key) we must NOT rip out its raw
                # sockets and abort an in-flight request; the helper only reaps
                # fds when the loop is dead.
