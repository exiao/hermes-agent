...
def _is_codex_gpt54_or_gpt55(model: Optional[str], provider: Optional[str] = None) -> bool:
    ...
    return bare == "gpt-daybreak-blue-latest" or any(
        ...
        for fam in ("gpt-5.4", "gpt-5.5", "gpt-5.6", "gpt-6-astra"))
...
def _is_unsupported_parameter_error(exc: Exception, param: str) -> bool:
    ...
    return param_lower in err_lower and _contains_any(err_lower, (
        ...
        "is deprecated", "deprecated for this model", "no longer supported",
    ))
...
def _main_route_target(runtime: Dict[str, Any], task: Optional[str]) -> Tuple[str, str, str, Any, str]:
    ...
    # A live runtime can supply only the provider while the model falls back to
    # config.yaml.  Pairing those independently sourced values sent the config's
    # Claude model to a Codex runtime.  Prefer a provider-owned auxiliary
    # default; when none exists, leave the model empty so Step 1 is skipped.
    skip_main_route = False
    if runtime.get("provider") and not runtime.get("model") and main_model:
        provider_default = _get_aux_model_for_provider(main_provider)
        if provider_default:
            logger.info(
                "Auxiliary auto-detect: runtime provider %s has no runtime model; "
                "using its registered default %r instead of config default %r",
                main_provider, provider_default, main_model,
            )
            main_model = provider_default
        else:
            logger.info(
                "Auxiliary auto-detect: runtime provider %s has no runtime model or "
                "registered auxiliary default; skipping the main-provider route",
                main_provider,
            )
            skip_main_route = True
...
def _resolve_auto(
    ...
    """Compatibility facade for callers that only need the client and model."""
...
def cleanup_stale_async_clients() -> None:
    ...
    with _client_cache_lock:
        ...
        for key, _client, _cached_loop in stale:
            del _client_cache[key]
    for _key, client, cached_loop in stale:
