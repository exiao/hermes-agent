...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    ...
    for source_name, reader in (
        ...
        if creds and creds.get("accessToken"):
            ...
            if configured_base := _get_configured_provider_base_url(seed.provider):
                payload["base_url"] = configured_base
            seed.upsert(source_name, payload)
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # Under gateway profile multiplexing a scope is installed; ``load_env`` and
    # os.environ both resolve the DEFAULT profile's values, which must NOT seed
    # (and then persist into) the requesting profile's credential pool. When a
    # scope is active, get_secret is authoritative and the .env / os.environ
    # fallbacks are dropped, so a scoped miss stays a miss instead of leaking +
    # writing another profile's key. Single-profile (no scope) behaviour is
    # unchanged.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    for env_var in env_vars:
        ...
        if configured_base := _get_configured_provider_base_url(provider):
