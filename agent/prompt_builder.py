...
KANBAN_GUIDANCE = (
    ...
    "reviewer can approve+unblock or request changes. **But \\\"review\\\" "
    "means a real human judgment is owed (a reviewer's verdict, conflicting "
    "requirements, a prod-deploy approval) — NOT \\\"a PR is waiting to be "
    "merged.\\\" A PR you opened that is green CI + approved + clean diff + "
    "all review threads resolved is your finished deliverable: call "
    "`kanban_complete` (it lands in `done`), do not block. Eric clicking "
    "merge is a click, not a decision, so \\\"awaiting Eric's merge\\\" is "
    "`done`, never `blocked`. This applies to every lane that opens a PR, "
    "not just the babysitter.** Reviewing-then-completing is more honest "
    "than auto-completing work that still needs eyes on it — but a "
    "merge-ready PR has already had its eyes.\\n"
