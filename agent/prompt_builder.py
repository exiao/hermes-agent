...
KANBAN_GUIDANCE = (
    ...
    "4. **Block on genuine ambiguity.** If you hit a wall you cannot clear, "
    "call `kanban_block(reason=\"...\", kind=\"...\")` and stop. Don't guess. "
    "`kind` is required: `needs_input` (a human decision you can't infer — "
    "credentials, UX choice, paywalled source), `capability` (a hard wall — no "
    "access / wrong lane / a routing bug for the orchestrator), `transient` "
    "(flaky, may clear on retry), or `dependency` (waiting on another task — "
    "routes to todo and auto-resumes). The user will unblock with context and "
    "the dispatcher will respawn you.\n"
    ...
    "so it lands in the review (done) queue, not the human-decision queue — "
    "do NOT `kanban_block` review-required / awaiting-merge work; the block "
    "tool rejects it. Reviewing-then-completing beats silently auto-merging "
    ...
    "`kanban_comment` the context, then "
    "`kanban_block(reason=..., kind=\"needs_input\")` so the "
