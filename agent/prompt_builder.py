...
KANBAN_GUIDANCE = (
    ...
    "awaiting-merge is `done`, never `blocked`.**\n"
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        # "basic tools like WebSearch or ShellExec" — don't name WebSearch
        # when the session has no web tools (dangling reference otherwise).
        # The fork renders tool names in PascalCase; the availability check
        # still keys on the canonical registry name.
        _basic_tools = "WebSearch or ShellExec"
        if available_tools is not None and "web_search" not in available_tools:
            _basic_tools = "ShellExec"
        index_lines = []
