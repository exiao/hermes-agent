...
from agent.skill_utils import (
    ...
    get_skills_preload_all, iter_skill_index_files, parse_frontmatter, read_active_org_id, skill_matches_environment,
...
KANBAN_GUIDANCE = (
    ...
    "detection.\n"
    "**A PR you opened that is green CI + approved + clean + threads resolved is your finished deliverable: "
    "`kanban_complete` it (lands in `done`); awaiting-merge is `done`, never `blocked`.**\n"
...
def _collect_extra_skills(
    root: Path, skill_files, hides, claimed: set[str], skills_by_category: dict[str, list[tuple[str, str, bool]]],
    ...
    for skill_file in skill_files:
        try:
            ...
            skills_by_category.setdefault(entry["category"], []).append((
                fm_name,
                f"{desc_prefix}{entry['description']}".strip(),
                bool(entry.get("preloaded", False)),
            ))
        except Exception as e:
            logger.debug(log_fmt, skill_file, e)


def _label_visible_entries(visible_entries: list[dict], skills_by_category: dict[str, list[tuple[str, str, bool]]]) -> None:
    ...
    for entry in visible_entries:
        ...
        skills_by_category.setdefault(category, []).append((
            fm, desc, bool(entry.get("preloaded", False))
        ))


def _render_skills_index(
    skills_by_category: dict[str, list[tuple[str, str, bool]]], category_descriptions: dict[str, str],
    ...
    *, preload_all: bool = False,
    ...
    for category in sorted(skills_by_category):
        ...
        seen = set()
        for name, desc, _preloaded in sorted(entries, key=lambda x: x[0]):  # stable: first entry per name wins
            if name not in seen:
                seen.add(name)
                index_lines.append(f"    - {name}: {desc}" if desc else f"    - {name}")
    has_preloaded = any(
        preloaded for entries in skills_by_category.values()
        for _name, _desc, preloaded in entries
    )
    if has_preloaded:
        preloaded_lines = []
        category_lines = []
        for category in sorted(skills_by_category):
            entries = skills_by_category[category]
            seen = set()
            for name, desc, preloaded in sorted(entries, key=lambda item: item[0]):
                if name in seen:
                    continue
                seen.add(name)
                if preloaded or preload_all:
                    preloaded_lines.append(f"    - {name}: {desc}" if desc else f"    - {name}")
            if not preload_all:
                count = len(seen)
                cat_desc = category_descriptions.get(category, "")
                category_lines.append(
                    f"    - {category} ({count} skills): {cat_desc}"
                    if cat_desc else f"    - {category} ({count} skills)"
                )
        index_lines = ["  ## Pre-loaded skills (load with skill_view):", *preloaded_lines]
        if not preload_all:
            index_lines.extend([
                "",
...
def _build_skills_system_prompt_inner(
    ...
    cache_key = (
        ...
        _platform_hint, tuple(sorted(disabled)), tuple(sorted(compact_categories or ())), preload_all,
    )
    with _SKILLS_PROMPT_CACHE_LOCK:
    ...
    result = _render_skills_index(
        skills_by_category, category_descriptions, compact_categories,
        available_tools, preload_all=preload_all,
    )
    with _SKILLS_PROMPT_CACHE_LOCK:
