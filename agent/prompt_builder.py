...
KANBAN_GUIDANCE = (
    ...
    "Skip heartbeats for short tasks. **If your task may run longer than 30 "
    "minutes, you MUST call `kanban_heartbeat` at least every 30 minutes** — the "
    "dispatcher reclaims tasks running past "
    "`kanban.dispatch_stale_timeout_seconds` when no heartbeat has arrived in "
    "the last 40 minutes. A reclaim re-queues the task as `ready` without "
    "penalty (no failure counter tick), but you lose your current run's "
    "progress.\n"
    ...
    "Exception: if your output is a code change that still needs human "
    "eyes before it counts as done, drop the structured metadata "
    "(changed_files / tests_run / diff_path) into a `kanban_comment` "
    "first, then end with "
    "`kanban_block(reason=\"review-required: <one-line summary>\")` so a "
    "reviewer can approve+unblock or request changes. **But \"review\" means "
    "a real human judgment is owed (a reviewer's verdict, conflicting "
    "requirements, a prod-deploy approval) — NOT a PR waiting to be merged. "
    "A PR you opened that is green CI + approved + clean + threads resolved "
    "is your finished deliverable: `kanban_complete` it (lands in `done`), "
    "do not block — every lane that opens a PR. Awaiting-merge is `done`, "
    "never `blocked`.**\n"
...
def build_skills_system_prompt(
    ...
    skills_by_category: dict[str, list[tuple[str, str, bool]]] = {}
    category_descriptions: dict[str, str] = {}
    ...
    if not skills_by_category:
        ...
        # Demoted categories (outside the current coding context) collapse to a
        # single names-only line so memory-anchored recall still works.
        if not has_any_preloaded:
            index_lines = []
            for category in sorted(skills_by_category.keys()):
                if category in demoted:
                    names = sorted({name for name, _desc, *_rest in skills_by_category[category]})
        ...
        result = (
            ...
            "skills, voice, gateway, plugins, or any feature — load the `assistant-runtime` skill "
            "first. It has the actual commands (e.g. `runtime config set …`, `runtime tools`, "
            "`runtime setup`) so you don't have to guess or invent workarounds.\n"
