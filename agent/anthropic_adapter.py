...
def _parse_claude_version(model: str) -> Optional[Tuple[int, int]]:
    ...
    (``claude-opus-4-8-20250715``). The minor component is 1-2 digits with a
    no-trailing-digit guard so an 8-digit date stamp on a minor-less name
    ...
    names (e.g. ``minimax``, ``qwen3``).

    Capability decisions live in the default-to-modern predicates below; this
    helper exists for callers/tests that want the raw numeric version.
    """
    import re
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    # Fork: local billing-proxy inbound key. Runs last so it merges on top of
    # whatever headers the auth/opencode branches above installed.
    kwargs = _apply_proxy_api_key_header(kwargs, normalized_base_url)
