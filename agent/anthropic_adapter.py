...
def _parse_claude_version(model: str) -> Optional[tuple[int, int]]:
    """Extract a Claude model's major and minor version."""
    normalized = model.lower().replace(".", "-")
    if "claude" not in normalized:
        return None
    match = re.search(r"claude-(?:[a-z]+-)?(\d+)-(\d{1,2})(?!\d)", normalized)
    if match:
...
def _build_anthropic_client_with_bearer_hook(
    ...
    return _new_sdk_client(
        sdk, kwargs, headers, target_base_url=normalized_base_url
    )


def _apply_proxy_api_key_header(
    headers: Dict[str, str], base_url: str | None
) -> Dict[str, str]:
    """Attach the fork billing-proxy admission key only to its own host."""
    proxy_api_key = _os.environ.get("PROXY_API_KEY")
    proxy_base_url = _os.environ.get("ANTHROPIC_BASE_URL") or ""
    proxy_host = base_url_hostname(proxy_base_url)
    target_host = base_url_hostname(base_url or proxy_base_url)
    if not proxy_api_key or not proxy_host or target_host != proxy_host:
        return headers
    return {**headers, "x-proxy-api-key": proxy_api_key}
...
def _new_sdk_client(
    ...
    *,
    ...
    headers = _apply_proxy_api_key_header(headers, target_base_url)
    if headers:
