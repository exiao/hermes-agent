...
def _sync_compressor_runtime(agent: Any) -> None:
    """Align the compressor with the resolved live route without clearing guards."""
    compressor = getattr(agent, "context_compressor", None)
    model = getattr(agent, "model", "") or ""
    if compressor is None or not model or not hasattr(compressor, "update_model"):
        return
    provider = getattr(agent, "provider", "") or ""
    base_url = getattr(agent, "base_url", "") or ""
    api_key = getattr(agent, "api_key", "") or ""
    api_mode = getattr(agent, "api_mode", "") or ""
    model_changed = getattr(compressor, "model", "") != model
    if not (
        model_changed
        or getattr(compressor, "provider", "") != provider
        or getattr(compressor, "base_url", "") != base_url
        or getattr(compressor, "api_key", "") != api_key
        or getattr(compressor, "api_mode", "") != api_mode
    ):
        return
    ...
    cooldown_deadline = float(
        getattr(compressor, "_summary_failure_cooldown_until", 0.0) or 0.0
    )
    cooldown_persist_failed = bool(
        getattr(compressor, "_cooldown_persist_failed", False)
    )
    ...
    if ineffective_count:
        compressor._ineffective_compression_count = ineffective_count
        ...
        if callable(getter) and callable(setter) and session_id:
            try:
                if not getter(session_id):
                    setter(session_id, ineffective_count)
            except Exception:
                compressor._ineffective_count_persist_failed = True
    if cooldown_deadline > time.monotonic():
        compressor._summary_failure_cooldown_until = cooldown_deadline
        compressor._cooldown_persist_failed = cooldown_persist_failed
...
def build_turn_context(
    ...
    try:
        _sync_compressor_runtime(agent)
    except Exception:
