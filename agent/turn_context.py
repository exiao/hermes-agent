...
def build_turn_context(
    ...
    *,
    ...
    # model/provider (after primary restoration above, so it never re-syncs to a
    # stale fallback backend). The compressor is built once at agent init,
    # sometimes before the live model/provider are known (e.g. cron workers where
    # it initialized with model="" provider=none). At compress time it passes its
    ...
    try:
        ...
        if _compressor is not None and _agent_model and hasattr(_compressor, "update_model"):
            ...
            if (_model_changed
                ...
                # Snapshot both the local breaker state and its durable mirror
                # before update_model() clears runtime calibration state.
                ...
                if callable(_pre_get) and _pre_id:
                    try:
                        _cooldown_before_sync = _pre_get(_pre_id)
                    except Exception:
                        _cooldown_before_sync = None
                _ineffective_before_sync = int(
                    getattr(_compressor, "_ineffective_compression_count", 0) or 0
                )
                ...
                if callable(_pre_get_ineffective) and _pre_id:
                    try:
                        _ineffective_before_sync = int(
                            _pre_get_ineffective(_pre_id) or _ineffective_before_sync
                        )
                    except Exception:
                        pass
                ...
                # update_model() clears the summary-failure cooldown when it
                # detects a runtime change (upstream #23767: a real model/
                # provider switch must give the new backend a fresh attempt).
                ...
                if _cooldown_snapshot:
                    ...
                    _sess_id = getattr(_compressor, "_session_id", "")
                    _still = None
                    _get_cd = getattr(_sess_db, "get_compression_failure_cooldown", None)
                    if callable(_get_cd) and _sess_id:
                        try:
                            _still = _get_cd(_sess_id)
                        except Exception:
                            _still = None
                    if not _still:
                        _rec = getattr(_sess_db, "record_compression_failure_cooldown", None)
                        if callable(_rec) and _sess_id:
                            try:
                                _rec(
                                    _sess_id,
                                    float(_cooldown_snapshot.get("cooldown_until") or 0.0),
                                    _cooldown_snapshot.get("error"),
                                )
                            except Exception:
                                pass
                        _rehydrate = getattr(
                            _compressor, "get_active_compression_failure_cooldown", None
                        )
                        if callable(_rehydrate):
                            try:
                                _rehydrate(refresh=True)
                            except TypeError:
                                _rehydrate()
