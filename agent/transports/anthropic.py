    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        for block in response.content:
            ...
            if block.type == "text":
                ...
                # Anthropic's single-underscore third-party classifier). Reverse
                # it back to the name the registry/dispatcher knows. Two original
                # forms map onto the same ``mcp__`` wire name:
                #   ``mcp__read_file``        <- bare native tool ``read_file``
                #   ``mcp__linear_get_issue`` <- MCP server tool
                #                                ``mcp_linear_get_issue``
                # Resolve by registry lookup, preferring whichever original is
                # actually registered; never rewrite a name that already resolves
                # natively. Gated on strip_tool_prefix (OAuth path) OR
                # registry_strip (main loop / local billing proxy path, where the
                # OAuth flag drifts False); call-scoped sampling paths pass
                # neither and a prefixed name is left untouched. GH-25255.
