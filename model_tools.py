...
def _worker_needs_kanban_tools() -> bool:
    """A real dispatcher worker must retain its lifecycle tool surface."""
    return bool(
        os.environ.get("HERMES_KANBAN_TASK")
        and not delegated_child_masks_kanban_ownership()
...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if enabled_toolsets is not None:
        ...
        if _worker_needs_kanban_tools() and "kanban" not in enabled:
    ...
    if disabled_toolsets:
        ...
        if _worker_needs_kanban_tools():
            disabled = [name for name in disabled if name != "kanban"]
        _apply_toolset_selection(tools, disabled, quiet_mode, disable=True)
    return tools
