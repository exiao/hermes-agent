...
def check_command_security(command: str) -> dict:
    ...
    # Strip known false-positive lookalike findings even when another finding
    # raised the overall verdict to block. An authoritative block consisting
    # only of the suppressed class remains blocked.
    if findings:
        kept = [finding for finding in findings if not _is_safe_lookalike_tld_finding(finding)]
        if not kept and action == "warn":
            return _verdict("allow")
        if kept and len(kept) != len(findings):
            findings = kept
...
def _is_safe_lookalike_tld_finding(finding: dict) -> bool:
    """Safe terminal .app/.dev TLD or trusted modal.run host finding."""
    ...
    if any(safe(finding.get(key)) for key in ("value", "tld", "detail", "description", "message")):
        return True
    evidence = finding.get("evidence")
    if isinstance(evidence, (list, tuple)):
        for item in evidence:
            if isinstance(item, dict):
                if any(safe(item.get(key)) for key in ("raw", "value", "url", "host", "domain")):
                    return True
            elif safe(item):
                return True
    return False
