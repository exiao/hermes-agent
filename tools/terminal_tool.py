...
from tools.terminal_tool_backends import (
    ...
    _get_modal_backend_state, _ManagedModalEnvironment, _ModalEnvironment,
)
...
_CONTAINER_CONFIG_DEFAULTS: Dict[str, Any] = {
    ...
    "container_persistent": True, "container_idle_timeout": 0, "lifetime_seconds": 300,
    "modal_mode": "auto", "vercel_runtime": "", "docker_volumes": [],
    "docker_mount_cwd_to_workspace": False, "docker_forward_env": [], "docker_env": {},
    "docker_run_as_host_user": False, "docker_extra_args": [], "docker_shm_size": "1g",
...
def build_container_config(env_type: str, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if env_type not in CONTAINER_ENV_TYPES:
        return None
    return {key: config.get(key, copy.deepcopy(default))
...
def _create_environment(*, env_type: str, image: str, cwd: str, timeout: int,
    ...
    """Upstream backend dispatcher with the fork's Modal idle-timeout plumbing."""
    if env_type != "modal":
        from tools.terminal_tool_backends import _create_environment as backend_create
    ...
    cc = build_container_config(env_type, container_config or {}) or {}
    sandbox_kwargs = {}
    cpu, memory, disk = cc["container_cpu"], cc["container_memory"], cc["container_disk"]
    if cpu > 0:
    ...
    if disk > 0:
        try:
            ...
            pass
    idle = _nonnegative_int(cc.get("container_idle_timeout"))
    if idle:
        sandbox_kwargs["idle_timeout"] = min(
            max(idle, _nonnegative_int(cc.get("lifetime_seconds")) * 2), timeout)
    state = _get_modal_backend_state(cc.get("modal_mode"))
    selected = state.get("selected_backend")
    if selected not in {"managed", "direct"}:
        from tools.terminal_tool_backends import _create_environment as backend_create
        return backend_create(
            env_type=env_type, image=image, cwd=cwd, timeout=timeout,
            ssh_config=ssh_config, container_config=cc, local_config=local_config,
    ...
    return cls(image=image, cwd=cwd, timeout=timeout, modal_sandbox_kwargs=sandbox_kwargs,
               persistent_filesystem=cc["container_persistent"], task_id=task_id)


def _create_configured_env(config: Dict[str, Any], env_type: str, *, image: str, cwd: str,
                           timeout: int, task_id: str, host_cwd: Optional[str], local_config=None):
    ...
    return _create_environment(
        ...
        container_config=build_container_config(env_type, config), local_config=local_config,
        task_id=task_id, host_cwd=host_cwd)
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "container_idle_timeout": _nonnegative_int(_tenv("TERMINAL_CONTAINER_IDLE_TIMEOUT", "0")),
