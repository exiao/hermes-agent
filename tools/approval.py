...
def _redact_for_approval(text: str) -> str:
    """Redact obvious credentials before showing approval prompts."""
    from agent.redact import redact_sensitive_text

    redacted = redact_sensitive_text(text or "")
    return _APPROVAL_SECRET_RE.sub("***", redacted)
...
# Self-process markers used by the self-termination guards. Four alternatives:
...
#      and the trailing `(?![\w])` keeps underscore lookalikes (`hermes_backend`,
#      `hermesd`, `hermes2`) out.
#   2. `hermes_cli` (+ optional `.main` etc.) — the agent's own module
#      entrypoint, which the hyphen-only alternative above deliberately excludes.
#      Killing `$(pidof hermes_cli.main)` is self-termination.
#   3. standalone `gateway` — the boundaries forbid a leading or trailing
#      word-char/hyphen so unrelated names (`api-gateway`, `gateway-ui`,
#      `gatewayd`) do not match, but a bare `gateway` process does.
#   4. `cli.py`, matched literally.
...
_SELF_PROC_TOKEN = (
    r'(?:(?<![\w-])hermes(?:-[\w.-]*)?(?![\w])|(?<![\w-])hermes_cli(?:[\w.]*)?|(?<![\w-])gateway(?![\w-])|cli\.py)'
)
...
DANGEROUS_PATTERNS = [
    ...
    # only when the pgrep/pidof target is the agent's own process — killing an
    # unrelated process (e.g. `kill $(pgrep -f metro)`) is a normal dev action
    # and must not be flagged. `pidof` is the BSD/Linux alternative to `pgrep`
    # and is equally opaque, so it shares the same scoped alternation.
    # Redirections are neutralized before matching (see _REDIRECT_TOKEN_RE) so a
    # redirect to a log path containing a self keyword is not a false positive.
    (r'\bkill\b[^;&|]*\$\(\s*(?:pgrep|pidof)\b[^)]*' + _SELF_PROC_TOKEN, "kill process via pgrep/pidof expansion (self-termination)"),
    (r'\bkill\b[^;&|]*`\s*(?:pgrep|pidof)\b[^`]*' + _SELF_PROC_TOKEN, "kill process via backtick pgrep/pidof expansion (self-termination)"),
...
def detect_dangerous_command(command: str) -> tuple:
    ...
    command_variants = list(_command_detection_variants(command))
    # Branch-aware force-push carve-out: if any normalized command view proves
    # this is a leased push to a non-protected feature branch, suppress blanket
    # force-push patterns across sibling views. Other patterns still apply.
    skip_force_push = any(
        _is_safe_lease_push_to_feature_branch(v.lower()) for v in command_variants
    )
    for command_variant in command_variants:
        command_lower = command_variant.lower()
        # A scan view used ONLY for self-termination patterns (redirect-stripped,
...
def _get_allow_execute_code() -> bool:
    ...
    if isinstance(val, str):
        return val.strip().lower() in {"true", "1", "yes", "on", "allow"}
    return bool(val)
    ...
    *,
    ...
    if is_gateway or env_var_enabled("HERMES_EXEC_ASK"):
        ...
        if notify_cb is not None:
            approval_data = {
                "command": _redact_for_approval(display_target),
                "pattern_key": pattern_key,
                "pattern_keys": [pattern_key],
                "description": _redact_for_approval(description),
        ...
        if not _should_fall_through_to_cli_approval(
            ...
            # Fork: the queued record and the agent-visible payload are
            # REDACTED — a pending approval is persisted and echoed back into
            # the transcript, so secrets in the command must not survive here.
            redacted_target = _redact_for_approval(display_target)
            redacted_description = _redact_for_approval(description)
            ...
            return {
                ...
                "description": redacted_description,
                "message": (
                    f"⚠️ This action is potentially dangerous ({redacted_description}). "
