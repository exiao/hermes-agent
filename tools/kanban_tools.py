...
def _clip_block_reason(reason: str) -> str:
    try:
        limit = int(cfg_get(load_config(), "kanban", "block_reason_max_chars",
    ...
    return reason[:limit - len(marker)].rstrip() + marker


KANBAN_BLOCK_SCHEMA["parameters"]["properties"]["reason"]["description"] += (
    " Long reasons are truncated; put run ids, paths, commands, and other detail in a kanban_comment.")
KANBAN_CREATE_SCHEMA["parameters"]["properties"]["model"]["description"] = (
    "Per-task model override. The dispatcher uses this model for this run only, overriding the "
    "assignee profile default. Omit to use the profile default.")
...
def _handle_create(args: dict, **kw) -> str:
    ...
    _check(not (assignee_error := _kb_validate.validate_assignee(assignee)), assignee_error or "invalid assignee")
