...
def _goal_mode_handoff_rejection(task, evidence: str) -> Optional[str]:
    ...
    if transport_failed:
        ...
        # error) — not a "not done" judgment. judge_goal returns
        # verdict="continue" here; treating that as a rejection wedges a
        # genuinely complete card on a transient proxy/aux blip (observed
        # live: a proxy account-pool 401 window blocked 3 merge-ready
        # PR-babysitter cards). Fail open.
        logger.warning(
