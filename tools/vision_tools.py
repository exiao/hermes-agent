    ...
    for attempt in range(max_retries):
        try:
            ...
            await _download_media(
                video_url, destination, 1, media_label="Video",
                accept="video/*,*/*;q=0.8", max_bytes=_MAX_VIDEO_BASE64_BYTES,
                timeout=60.0, retry_all=True)

            # The streaming download above enforces the size cap and re-checks
            # website access on the FINAL url, but does not look at the bytes.
            # A watch/share page returns 200 with HTML; without this guard that
            # HTML reaches the model, which silently hallucinates an answer
            # from garbage input.
            head = b""
            try:
                with destination.open("rb") as _f:
                    head = _f.read(64)
            except OSError:
                head = b""
            if not _looks_like_video_bytes(head):
                try:
...
async def _materialize_video(video_url: str, task_id: Optional[str], temp_paths: list) -> Path:
    ...
    if local_path.is_file():
        ...
        return local_path
    if _is_streaming_host(video_url) and not _is_direct_media_url(video_url):
        blocked = check_website_access(video_url)
        if blocked:
            raise PermissionError(blocked["message"])
        path = get_hermes_dir("cache/video", "temp_video_files") / f"temp_video_{uuid.uuid4()}.mp4"
        path = await _extract_stream_to_file(video_url, path)
        temp_paths.append(path)
        return path
    if await _validate_image_url_async(video_url):
        ...
        path = get_hermes_dir("cache/video", "temp_video_files") / f"temp_video_{uuid.uuid4()}{suffix}"
        temp_paths.append(path)
        return await _download_video(video_url, path)
