...
_TOOLSET_ALIASES = {
    ...
    "sessionsearch": "session_search", "filesystem": "file", "files": "file",
    "code": "code_execution", "codeexecution": "code_execution",
    "web_search": "search", "websearch": "search",
    "image": "image_gen", "images": "image_gen",
}


def _get_toolset_canonical_map() -> Dict[str, str]:
    result = {str(name).lower(): str(name) for name in get_toolset_names()}
    result.update({"all": "all", "*": "all"})
    static = {str(name).lower() for name in TOOLSETS}
    try:
        from tools.registry import registry
        for name in registry.get_registered_toolset_names():
            if str(name).lower() not in static:
                result[str(name).lower()] = str(name)
        for alias, canonical in registry.get_registered_toolset_aliases().items():
            if canonical and str(alias).lower() not in static:
                result[str(alias).lower()] = str(canonical)
    except Exception:
        pass
    return result
...
def _normalize_toolset_names(names):
    ...
    for raw in names:
        ...
        canonical = valid.get(lowered) or _TOOLSET_ALIASES.get(lowered)
        if canonical is None and lowered.startswith("mcp_"):
            unprefixed = lowered[4:]
            canonical = (valid.get(unprefixed) or valid.get(f"mcp-{unprefixed}")
                         or _TOOLSET_ALIASES.get(unprefixed))
        if canonical is None:
            logger.warning("delegate_task: unknown toolset %r ignored", raw)
...
def _is_mcp_toolset(name: str) -> bool:
    ...
    try:
        from tools.registry import registry
        return str(registry.get_toolset_alias_target(str(name)) or "").startswith("mcp-")
    except Exception:
        return False


def _resolve_child_toolsets(parent_agent, toolsets, effective_role):
    ...
    if parent_enabled is not None:
        ...
        import model_tools
        parent_toolsets = {
            toolset for name in parent_agent.valid_tool_names
    ...
    expanded.update(canonical[str(name).lower()] for name in parent_toolsets
                    if str(name).lower() in canonical)
    parent_tools = {
        tool for name in expanded for tool in (TOOLSETS.get(name) or {}).get("tools", [])}
    expanded.update(
        name for name, definition in TOOLSETS.items()
        if definition.get("tools") and set(definition["tools"]).issubset(parent_tools))
    if toolsets is not None:
        child_toolsets = _strip_blocked_tools([
            name for name in (_normalize_toolset_names(toolsets) or []) if name in expanded])
        if child_toolsets and is_truthy_value(_load_config().get("inherit_mcp_toolsets"), default=True):
            for name in sorted(parent_toolsets):
                canonical_name = canonical.get(str(name).lower(), name)
                if _is_mcp_toolset(canonical_name) and canonical_name not in child_toolsets:
                    child_toolsets.append(canonical_name)
    elif parent_enabled is not None:
        child_toolsets = _strip_blocked_tools(list(parent_enabled))
    else:
        child_toolsets = _strip_blocked_tools(sorted(parent_toolsets) or DEFAULT_TOOLSETS)
    disabled = getattr(parent_agent, "disabled_toolsets", None)
    inherited = [str(name) for name in disabled] if isinstance(disabled, (list, tuple, set)) else []
    if effective_role == "orchestrator":
        inherited = [name for name in inherited if name != "delegation"]
        if "delegation" not in child_toolsets:
            child_toolsets.append("delegation")
    child_disabled = list(dict.fromkeys(
        inherited + _blocked_toolsets_for_role(effective_role) + ["kanban"]))
    return child_toolsets, child_disabled


def _configured_child_fallbacks(config: dict) -> Optional[List[Dict[str, str]]]:
    """Explicit fallbacks for a provider-pinned child; malformed entries are ignored."""
    raw = config.get("fallback_providers")
    if not isinstance(raw, list):
        return None
    result = [dict(entry) for entry in raw if isinstance(entry, dict)
