...
def _build_child_agent(
    ...
    if os.environ.get("HERMES_KANBAN_TASK"):
        ...
        if "kanban" not in child_disabled_toolsets:
    ...
    # A delegated child inherits the parent worker's ``HERMES_KANBAN_TASK`` env,
    # and ``AIAgent(...)`` resolves the child's tool set DURING construction
    # (here, on the parent thread) — before ``run_conversation`` engages the
    # ownership mask at call time. Engage it around construction so
    # ``get_tool_definitions`` can tell this kanban-worker child (which must NOT
    # receive lifecycle tools) apart from a real worker whose profile merely
    # disabled kanban for cost. Non-kanban parents build unchanged.
    # Upstream's ``delegated_child_context`` marks child lineage generally and is
    # a SEPARATE ContextVar — both wrap construction.
    from contextlib import nullcontext

    from agent.delegation_context import delegated_child_context

    _child_build_ctx = (
        delegated_child_kanban_env()
        if os.environ.get("HERMES_KANBAN_TASK")
        else nullcontext()
    )
    with delegated_child_context(), _child_build_ctx:
        child = AIAgent(
    ...
    **_kwargs,
    ...
    try:
        ...
        def _run_with_thread_capture():
            ...
            with delegated_child_context(), delegated_child_kanban_env():
