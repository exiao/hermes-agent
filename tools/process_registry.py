...
from tools.process_registry_notifications import format_process_notification as _format_process_notification


def format_process_notification(evt: dict) -> "str | None":
    """Do not render a recovered process's unknown exit status as ``exit code None``."""
    if evt.get("type") == "completion" and evt.get("exit_code") is None:
        evt = {**evt, "exit_code": "unavailable"}
    return _format_process_notification(evt)
        ...
        try:
            with self._lock:
                ...
                for s in self._running.values():
                    ...
                    # See #77484.
                    entry["command"] = redact_sensitive_text(s.command, code_file=True, force=True)
...
def _redact_process_result(result: dict) -> dict:
    ...
    if isinstance(command, str) and command:
        result["command"] = redact_sensitive_text(command, code_file=True, force=True)
    return result
