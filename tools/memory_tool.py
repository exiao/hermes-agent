...
from tools.memory_tool_store import (  # noqa: E402,F401  (re-exports)
    ENTRY_DELIMITER, MEMORY_BLOCK_HEADERS, MemoryStore as _BaseMemoryStore, _scan_memory_content)


def _spill_norm_tokens(text: str) -> set[str]:
    return {token for token in "".join(
        char if char.isalnum() else " " for char in text.lower()).split() if len(token) > 2}
...
def _spill_too_similar(content: str, existing_contents, threshold: float = 0.6) -> bool:
    left = _spill_norm_tokens(_spill_strip_metadata(content))
    if not left:
        return False
    for existing in existing_contents:
        right = _spill_norm_tokens(_spill_strip_metadata(existing))
        if right and len(left & right) / len(left | right) >= threshold:
            return True
    return False


def _spill_to_pending(target: str, content: str) -> str:
    """Append one flattened, same-target-deduplicated record for memory-gc."""
    try:
        ...
        if path.exists():
            for line in path.read_text(encoding="utf-8").splitlines():
                stored_target, separator, stored = line.partition("\t")
                if separator and stored_target == target:
                    same_target.append(stored)
        key = _spill_strip_metadata(flat)[:80].lower()
        if any(key in existing.lower() for existing in same_target) or _spill_too_similar(flat, same_target):
            return "deduped"
        with path.open("a", encoding="utf-8") as handle:
            handle.write(f"{target}\t{flat}\n")
        return "written"
    except Exception:
...
class MemoryStore(_BaseMemoryStore):
    """Upstream store with lossless overflow into the pending GC queue."""
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        def add_or_spill(entries, limit):
            ...
            if spill_status == "error":
                return self._consolidation_failure({
                    "success": False,
                    "error": (f"Hot memory full at {current:,}/{limit:,} chars and spilling to "
                              ".pending.md failed. The entry was NOT saved."),
                    "usage": f"{current:,}/{limit:,}", "spilled_to_pending": False})
            spilled = spill_status == "written"
            note = ("queued to .pending.md; memory-gc will place it tonight" if spilled
                    else "already queued or near-duplicate of a pending entry")
            return {"success": True,
                    "message": f"Hot memory full at {current:,}/{limit:,} chars — entry {note}.",
                    "usage": f"{current:,}/{limit:,}", "spilled_to_pending": spilled}

        return self._mutate(target, add_or_spill, skip_drift=True)
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "stay small.\n\n"
        "IF FULL: an add is queued to episodes/.pending.md for memory-gc; use ONE batch to "
        "remove or shorten stale entries first when the fact must surface immediately.\n\n"
