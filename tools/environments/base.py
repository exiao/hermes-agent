...
# The upstream decomposition moved this adapter to ``base_output``. Replace
# the exported class here so every SDK backend retains the fork's diagnostic
# contract: transport failures appear in stdout instead of a bare exit 1.
from tools.environments import base_output as _base_output


class _ThreadedProcessHandle:
    """SDK ``exec_fn`` adapter that surfaces backend exceptions to the caller."""
        ...
        def _worker():
            try:
                ...
                try:
                    os.write(
                        self._write_fd,
                        f"[backend error] {type(exc).__name__}: {exc}".encode("utf-8", errors="replace"),
                    )
                except OSError:
                    pass
            finally:
...
_base_output._ThreadedProcessHandle = _ThreadedProcessHandle
...
class BaseEnvironment(ABC):
    ...
    def _snapshot_excluded_passthrough_names(self) -> tuple[str, ...]:
        ...
        self._snapshot_passthrough_names.add("HERMES_DELEGATED_CHILD_CONTEXT")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        """Attach stdin to a brace group containing the whole command sequence."""
    ...
    def execute(
        ...
        *,
        ...
        def _spawn_and_wait() -> dict:
            ...
            import inspect
            wait_kwargs = {"timeout": effective_timeout, "bounded_capture": bounded_capture}
            params = inspect.signature(self._wait_for_process).parameters
            if "watch_interrupt_tid" in params or any(
                    p.kind == inspect.Parameter.VAR_KEYWORD for p in params.values()):
                wait_kwargs["watch_interrupt_tid"] = parent_tid
            if yield_handler is not None:
                wait_kwargs["yield_handler"] = yield_handler
            return self._wait_for_process(spawned, **wait_kwargs)
