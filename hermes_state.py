...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def append_message(
        ...
        def _do(conn):
            # Upstream guards run FIRST so a busy/closed compression session
            # still raises even when the incoming row happens to be a re-persist.
    ...
    def _insert_message_rows(
        ...
        *dedupe* controls the re-persist idempotency guard below. Callers that
        ...
        for msg in messages:
            ...
            if _dup is not None:
                # Re-persist of an existing row: hand back the EXISTING row id
                # so upstream's ``msg["_row_id"]`` contract still holds for
                # deduped messages (a caller that keys on _row_id would
                # otherwise see it missing only on the dedupe path).
                if isinstance(msg, dict):
                    msg["_row_id"] = _dup[0]
                continue
    ...
    def replace_messages(
        ...
        def _do(conn):
            ...
            total_messages, total_tool_calls = self._insert_message_rows(
                conn, session_id, messages, dedupe=not active_only
            )
            conn.execute(
                "UPDATE sessions SET message_count = ?, tool_call_count = ? WHERE id = ?",
