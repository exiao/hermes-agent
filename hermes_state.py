...
_MESSAGE_ROW_DEDUPE: ContextVar[bool] = ContextVar("hermes_message_row_dedupe", default=True)
...
class SessionDB(
    ...
    def append_message(
        ...
        codex_message_items: Any = None, platform_message_id: str = None,
        observed: bool = False, effect_disposition: Optional[str] = None,
        ...
        """Append one message, idempotently when a persisted row is replayed."""
        from hermes_state_messages import (
            _INSERT_MESSAGE_SQL, _coerce_timestamp, _parse_tool_calls, _tool_calls_count)
        ...
        params = self._message_row_params(
            session_id, role, msg, parsed_tool_calls,
            _coerce_timestamp(timestamp, time.time()), keep_reasoning=True)

        def _do(conn):
            # Admission/lease guards remain first: dedupe must never bypass a
            # closed compression parent or a lost turn lease.
            ...
            duplicate = conn.execute(
                """SELECT id FROM messages
                   ...
                   LIMIT 1""",
                (params[0], params[1], params[7], params[2], params[3], params[4]),
            ).fetchone()
            if duplicate is not None:
                return duplicate[0]
            msg_id = conn.execute(_INSERT_MESSAGE_SQL, params).lastrowid
            self._bump_session_counters(
                conn, session_id, 1, _tool_calls_count(parsed_tool_calls), unit=True)
            return msg_id
    ...
    def _insert_message_rows(
        ...
        """Insert fresh rows, skipping exact re-persisted natural-key clones."""
        from hermes_state_messages import (
            _INSERT_MESSAGE_SQL, _coerce_timestamp, _parse_tool_calls, _tool_calls_count)
    ...
    def replace_messages(
        ...
        token = _MESSAGE_ROW_DEDUPE.set(not active_only)
        try:
            return super().replace_messages(
            ...
            _MESSAGE_ROW_DEDUPE.reset(token)

    def archive_and_compact(
        self, session_id: str, compacted_messages: List[Dict[str, Any]],
        model_config_patch: Optional[Dict[str, Any]] = None,
        watermark: Optional[int] = None, lock_holder: Optional[str] = None,
        tail_count: int = 0,
    ) -> int:
        token = _MESSAGE_ROW_DEDUPE.set(False)
        try:
            return super().archive_and_compact(
                session_id, compacted_messages,
                model_config_patch=model_config_patch, watermark=watermark,
                lock_holder=lock_holder, tail_count=tail_count)
        finally:
            _MESSAGE_ROW_DEDUPE.reset(token)
