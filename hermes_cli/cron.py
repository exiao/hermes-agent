...
def _builtin_gateway_liveness() -> Optional[bool]:
    ...
    try:
        ...
        try:
            ...
            # tri-state to None.
            pass
        # Fork (#244): the self-inclusive PID file still answers when the lock
        # probe is unavailable. find_gateway_pids() below excludes the caller
        # and its whole ancestor chain, so inside the gateway process tree it
        # hides the very PID that proves the ticker is alive.
        try:
