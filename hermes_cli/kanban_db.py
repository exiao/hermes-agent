...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    try:
        ...
        return None
    # A NEW backup landed on disk — also enforce upstream's retention cap so
...
def set_status_direct(
    ...
    (user yanking a stuck worker back to the queue), and the reclaimed
    worker process is terminated after the transaction commits.

    A ``running -> ready`` move whose run was resuming a ``review`` phase
    lands back in ``review`` (or ``todo`` when parents aren't satisfied),
    so a manual yank doesn't silently drop the review requirement.

    This is the single sanctioned manual-move path shared by the dashboard
    plugin and the standalone kanban viewer — neither reimplements it nor
    writes ``UPDATE tasks SET status`` directly.
    """
    ...
    with write_txn(conn):
        ...
        if reopening_satisfied_parent:
            # A parent leaving done/archived invalidates every descendant that
            # ran on the strength of that completion. The shared domain
            # implementation records per-descendant events + comments, closes
            # their runs and resets failure counters, and hands back the worker
            # terminations to perform AFTER commit (events must be durable
            # before the kill).
...
def _dependency_wait_should_park(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    for r in conn.execute(
        ...
        if is_terminal:
            ...
            #
            # A post-wait ``completed`` event ALSO counts even when the replay
            # thinks the parent was already terminal: a reopen can move a
            # parent off done without emitting an event (a direct
            # ``UPDATE tasks SET status`` from a repair path or a caller that
            # bypasses set_status_direct), which makes the replay's state
            # stale. Re-completion after the wait is unambiguously a new
            # resolution. ``archived`` is deliberately NOT included — routine
            # cleanup archives an already-done parent and resolves nothing.
            if r["id"] > dep_evt_id and (
                became_terminal or r["kind"] == "completed"
            ):
                newly_resolved = True
                break
        else:
...
def complete_task(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        ev_lines = (event_summary or "").strip().splitlines()
...
def edit_completed_task_result(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        ev_lines = (handoff_summary or "").strip().splitlines()
        ev_summary = ev_lines[0][:_EVENT_PAYLOAD_DETAIL_MAX] if ev_lines else ""
        _append_event(
            conn, task_id, "edited",
            {
