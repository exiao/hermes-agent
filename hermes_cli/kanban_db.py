...
# Terminal-event handoff payloads must stay larger than the notifier's 3,500-char
# visible cap so truncation is advertised there instead of happening silently at
# the producer.  The durable run row retains the same bounded detail.
...
def _resolve_lane_goal_defaults(
    ...
    """Resolve task-explicit or lane-default goal mode for one worker spawn."""
    cfg = _load_kanban_cfg() if kanban_cfg is None else (kanban_cfg or {})
    configured_lanes = cfg.get("goal_mode_lanes")
    lanes = (
        {str(item).strip().lower() for item in configured_lanes if str(item).strip()}
        if isinstance(configured_lanes, (list, tuple))
        else {item.lower() for item in DEFAULT_GOAL_MODE_LANES}
    )
    enabled = bool(task.goal_mode) or str(task.assignee or "").strip().lower() in lanes
    if not enabled:
        return False, None
    raw_turns = (
        task.goal_max_turns
        if task.goal_max_turns is not None
        else cfg.get("goal_mode_default_max_turns")
    )
    return True, _positive_config_int(raw_turns, DEFAULT_GOAL_MODE_MAX_TURNS)


def _resolve_worker_max_iterations(
    goal_mode: bool, kanban_cfg: Optional[dict] = None,
) -> Optional[int]:
    """Configured per-run cap for non-goal workers; absent/invalid leaves the default."""
    if goal_mode:
        return None
    cfg = _load_kanban_cfg() if kanban_cfg is None else (kanban_cfg or {})
    raw = cfg.get("worker_max_iterations")
    if raw is None:
        return None
    try:
        value = int(raw)
...
class KanbanFDPressureError(RuntimeError):
    """The board cannot be opened safely near a file-descriptor ceiling."""
...
def _resolve_fd_headroom() -> int:
    ...
    try:
        ...
        raw = (load_config_readonly().get("kanban") or {}).get("fd_headroom")
        if raw is not None:
            return int(raw)
...
def _count_open_fds() -> Optional[int]:
    for fd_dir in (f"/proc/{os.getpid()}/fd", "/dev/fd"):
        try:
            ...
            return 1 << 30 if exc.errno in (23, 24) else None
    return None
...
def _proc_fd_headroom() -> Optional[int]:
    try:
        ...
        opened = _count_open_fds()
        if soft in (resource.RLIM_INFINITY, -1, 0) or opened is None:
            return None
        return int(soft) - opened
    ...
    if sys.platform == "darwin":
        try:
            ...
            def _sysctl(name: str) -> Optional[int]:
                ...
                return int(value.value) if libc.sysctlbyname(
                    name.encode(), ctypes.byref(value), ctypes.byref(size), None, 0) == 0 else None
            maximum, used = _sysctl("kern.maxfiles"), _sysctl("kern.num_files")
            return maximum - used if maximum and used is not None else None
        except Exception:
            return None
    if sys.platform.startswith("linux"):
        try:
            allocated, _unused, maximum = Path("/proc/sys/fs/file-nr").read_text(encoding="utf-8").split()
            return int(maximum) - int(allocated) if int(maximum) > 0 else None
        except Exception:
            return None
    return None
...
def _assert_fd_headroom() -> None:
    required = _resolve_fd_headroom()
    if required <= 0:
        return
    candidates = [(value, scope) for value, scope in (
        (_proc_fd_headroom(), "per-process"), (_system_fd_headroom(), "system-wide"))
        if value is not None]
    if candidates:
        tightest, scope = min(candidates, key=lambda item: item[0])
        if tightest < required:
            raise KanbanFDPressureError(
                f"refusing kanban connect: {scope} FD headroom {tightest} < {required} required; "
                "opening the WAL/SHM board now risks index-desync corruption. Skipping this tick.")
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Latest archive event; keep completion time stable across re-archive.
    archived_at          INTEGER,
...
def validate_assignee(assignee: Optional[str]) -> Optional[str]:
    """Return an actionable error when a named assignee has no profile lane."""
    if assignee is None:
        return None
    ...
    return (
        ...
        f"spawns profiles under ~/.hermes/profiles/. Valid profiles: {valid}. "
        "(Use 'none' to create an unassigned card.)")
...
def _strip_workspace_scheme(
    ...
    match = _WORKSPACE_SCHEME_RE.match(workspace_path.strip())
    if not match:
        return workspace_kind, workspace_path
    if not workspace_kind or workspace_kind == "scratch":
        workspace_kind = match.group(1)
    return workspace_kind, match.group("path").strip() or None


def _worktree_path_resolvable(workspace_path: str) -> bool:
    """Accept repo roots, linked-worktree roots, and pending targets under a repo."""
    try:
        from hermes_cli.kanban_db_workspace import (
            _git_toplevel, _is_linked_worktree_checkout, _repo_root_for_worktree_target)
        ...
        if requested.exists():
            if _is_linked_worktree_checkout(requested):
                return True
            root = _git_toplevel(requested)
            return root is not None and requested.resolve(strict=False) == root
...
def _default_max_runtime_seconds() -> Optional[int]:
    try:
        from hermes_cli.config import load_config_readonly
        raw = (load_config_readonly().get("kanban") or {}).get("default_max_runtime_seconds")
...
def _is_spec_less(title: str, body: Optional[str], assignee: Optional[str]) -> bool:
    ...
    return (not normalized or normalized in _PLACEHOLDER_TITLES
...
def create_task(
    ...
    workspace_kind, workspace_path = _strip_workspace_scheme(workspace_kind, workspace_path)
    workspace_kind = workspace_kind or "scratch"
    if isinstance(workspace_path, str) and not workspace_path.strip():
        workspace_path = None
    ...
    if project_repo is None and workspace_kind in {"dir", "worktree"}:
        ...
        if workspace_kind == "worktree" and not _worktree_path_resolvable(str(candidate)):
            raise ValueError(
                f"workspace_kind='worktree' path {workspace_path!r} is not inside a git repo "
            ...
            with write_txn(conn, allow_nested=True):
                ...
                conn.execute(
                    ...
                    (
                        ...
                        branch_name, project_id, tenant, idempotency_key,
                        _opt_int(effective_max_runtime),
...
def list_tasks(
    ...
    idempotency_key: Optional[str] = None, include_archived: bool = False,
    ...
    excluded = tuple(dict.fromkeys(str(s) for s in exclude_statuses if s))
    unknown = set(excluded) - VALID_STATUSES
    if unknown:
        raise ValueError(f"exclude_statuses contains unknown status: {sorted(unknown)}")
    if excluded:
        query += " AND status NOT IN (" + ",".join("?" for _ in excluded) + ")"
...
def _completed_event_payload(
    ...
    """``completed`` event payload: first summary line (bounded for transport) so gateway
    ...
    payload: dict = {
        ...
        "summary": _first_line(event_summary, _EVENT_PAYLOAD_DETAIL_MAX) or None,
    }
    if verified_cards:
    ...
    with write_txn(conn):
        ...
        _append_event(
            conn, task_id, "edited",
            {
                ...
                "result_len": len(result) if result else 0,
                "summary": _first_line(handoff_summary, _EVENT_PAYLOAD_DETAIL_MAX) or None,
            },
            run_id=run_id,
        )
    return True
...
def _insert_decomposed_child(
    ...
    if child_ws_kind in {"dir", "worktree"} and not child_ws_path:
        board_default = _board_meta_for(None).get("default_workdir")
        if child_ws_kind == "dir" and board_default:
            child_ws_path = str(board_default)
        elif child_ws_kind == "worktree" and board_default:
            # Keep NULL so dispatch creates a task-specific worktree from the
            # board's repo anchor rather than sharing the checkout.
            child_ws_path = None
        else:
            raise ValueError(
    ...
    child_status = (
        "triage" if _is_spec_less(child["title"], body, assignee or _default_assignee())
        else "todo")
    conn.execute(
        "INSERT INTO tasks "
        ...
        (
            ...
            assignee, child_status, child_ws_kind, child_ws_path,
...
_migrate_upstream = _kanban_db_connect._migrate_add_optional_columns


def _migrate_with_fork_columns(conn: sqlite3.Connection) -> None:
    _migrate_upstream(conn)
    ...
    conn.execute(
        """UPDATE tasks SET archived_at = (
               ...
               WHERE task_events.task_id = tasks.id AND task_events.kind = 'archived')
...
_connect_upstream = _kanban_db_connect.connect


def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    """FD-guarded wrapper around the decomposed connection implementation."""
    _assert_fd_headroom()
    return _connect_upstream(db_path=db_path, board=board)


# The helper's connect_closing() resolves its module global at call time, so
# this keeps every direct/helper caller on the same corruption guard.
_kanban_db_connect.connect = connect
connect_closing = _kanban_db_connect.connect_closing
