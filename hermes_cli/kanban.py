...
from hermes_cli.kanban_parser import build_parser as _build_parser  # re-exported below


def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    """Build the upstream parser, retaining ``--dedupe-key`` as the public name.

    ``--idempotency-key`` remains a permanent alias for scripts already using
    it; both spellings populate ``args.dedupe_key``.
    """
    ...
    if command_action is None:
        return parser
    for command in ("create", "swarm"):
        subparser = command_action.choices.get(command)
        if subparser is None:
            continue
        action = next(
            (a for a in subparser._actions if "--idempotency-key" in a.option_strings), None)
        if action is None:
            continue
        action.option_strings.insert(0, "--dedupe-key")
        action.dest = "dedupe_key"
        subparser._option_string_actions["--dedupe-key"] = action
    list_parser = command_action.choices.get("list")
    if list_parser is not None and "--idempotency-key" not in list_parser._option_string_actions:
        list_parser.add_argument(
            "--idempotency-key", metavar="KEY",
            help="Restrict to tasks with this exact internal idempotency key")
    return parser
...
def _task_to_list_dict(task: kb.Task) -> dict:
    result = _task_to_dict(task)
    result["idempotency_key"] = task.idempotency_key
    result["last_heartbeat_at"] = task.last_heartbeat_at
    return result
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    if assignee_error:
        return _err(f"kanban: {assignee_error}", 2)
    with kbc.connect_closing() as conn:
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if _json_out(args, [_task_to_list_dict(t) for t in tasks]):
        return 0
...
def _goal_mode_handoff_rejection(task: Optional[kb.Task], evidence: str):
    ...
    if transport_failed:
        ...
        _logging.getLogger(__name__).warning(
            "goal judge unreachable (%s); allowing lifecycle handoff", reason)
        return ("done", None)
