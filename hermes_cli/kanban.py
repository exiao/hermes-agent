...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ...
        # Fetch the graph context INSIDE the connection block. The diagnostics
        # section below runs after `connect_closing` has closed `conn`, so
        # querying there raises sqlite3.ProgrammingError ("Cannot operate on a
        # closed database") and swallows the rest of show's output (comments,
        # runs, events).
        graph_context = kb.task_graph_context(conn, task.id)
    ...
    diags = kd.compute_task_diagnostics(
        task, events, runs, graph=graph_context
    )
    if diags:
...
def _goal_mode_handoff_rejection(task: Optional[kb.Task], evidence: str) -> Optional[str]:
    ...
    try:
        ...
        # transport_failed) — see hermes_cli/goals.py. Unpacking fewer raises
        # ValueError into the handler below, silently disabling the gate.
    ...
    if transport_failed:
        # The judge could not be REACHED (auth 401, timeout, DNS, connection
        # error), not a "not done" judgment. Fail open, mirroring the tool-call
        # gate in tools/kanban_tools.py — a transient aux/proxy blip must not
        # wedge a genuinely complete card.
        import logging as _logging
