...
class CLIChatTurnMixin:
    ...
    def _chat_run_agent(self, turn, message):
        ...
        try:
            ...
            self._last_failure_reason = (
                turn.result.get("failure_reason")
                if turn.result and turn.result.get("failed")
                else None
            )
    ...
    def _chat_settle_turn(self, turn):
        ...
        # Non-quiet single-query callers do not retain the result dict. Keep the
        # classified failure reason so a Kanban worker can map a transient quota
        # wall to EX_TEMPFAIL after normal session finalization.
        self._last_failure_reason = (
            turn.result.get("failure_reason")
            if turn.result and turn.result.get("failed")
            else None
