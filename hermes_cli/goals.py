...
KANBAN_GOAL_CONTINUATION_TEMPLATE = (
    ...
    "need human input, call kanban_block with a reason. If a PR is CLEAN or "
    "BLOCKED with green CI and no unresolved review threads, and "
    "reviewDecision is REVIEW_REQUIRED, first rule out other merge blockers; "
    "only if approval is the remaining gate you cannot obtain, call "
    "kanban_block(kind=\"needs_input\", reason=...) instead of consuming "
    "another continuation. Do not stop without calling one of them."
)
...
KANBAN_GOAL_FINALIZE_TEMPLATE = (
    ...
    "summary of what you did. If it is a code change awaiting same-card "
    "review, call kanban_request_review with that summary instead. If "
    "something still blocks completion, call kanban_block with the reason "
    "instead. If this is a PR that is CLEAN or BLOCKED with green CI and no "
    "unresolved review threads, and reviewDecision is REVIEW_REQUIRED, first "
    "rule out other merge blockers; only if approval is the remaining gate "
    "you cannot obtain, call kanban_block(kind=\"needs_input\", reason=...) "
    "instead."
)
