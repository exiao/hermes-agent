...
def _resolve_copilot_raw_token_from_active_scope(
    ...
    """Resolve Copilot from the active profile scope, if one is installed.

    A named profile's scoped miss is authoritative: falling through to
    ``gh auth token`` would borrow the process/default profile identity. The
    default profile may still use that process identity.
    """
    ...
    try:
        ...
        if get_hermes_home().parent.name == "profiles":
            return "", ""
    except Exception:
        pass
    return None
...
def _resolve_copilot_raw_token(pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if current_secret_scope() is None and is_multiplex_active():
        return "", ""
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    for env_var in pconfig.api_key_env_vars:
        if current_secret_scope() is None and not is_multiplex_active():
            raw = get_env_value_prefer_dotenv(env_var)
        else:
            try:
                raw = get_secret(env_var, "")
            except UnscopedSecretError:
                continue
        val = _usable_declared_secret(provider_id, raw, env_var)
        if val:
    ...
    # An unscoped multiplex caller must not seed/read the default profile's
    # credential pool after its environment lookup correctly failed closed.
    if current_secret_scope() is None and is_multiplex_active():
        return "", ""
...
def _copilot_runtime_base_url(api_key: str, default: str, env_url: str) -> str:
    ...
    try:
        from hermes_cli.copilot_auth import get_copilot_api_token
        raw_token, _ = _resolve_copilot_raw_token(PROVIDER_REGISTRY["copilot"])
        if raw_token:
