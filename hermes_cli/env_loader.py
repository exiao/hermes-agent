    ...
    *writes* pass through to the real ``os.environ`` (so the load still takes
    ...
    def _encode_key(self, key: Any) -> Any:
        ...
        try:
            ...
            # Wrong key type (bytes on a str-keyed environ). Re-raise so this
            # wrapper agrees with the real mapping; falling back to the raw key
            # aliased ``stable[b'PATH']`` onto ``'PATH'``, so os.get_exec_path
            # saw both spellings and killed every subprocess spawned here.
            raise
    ...
    with _ENV_LOAD_LOCK:
        ...
        try:
            try:
                ...
                load_dotenv(
                    stream=io.StringIO(raw.decode("latin-1")), override=override
                )
        finally:
...
def load_hermes_dotenv(
    *,
    ...
    # Fork policy: the root .env is the shared secret base for named profiles;
    # the profile file may override it. Derive from the explicit home argument
    # so isolated/test homes never inherit the operator's real credentials.
    root_env: Path | None = None
    try:
        from hermes_constants import get_default_hermes_root
        candidate = get_default_hermes_root(home_path) / ".env"
