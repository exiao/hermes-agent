...
def _record_task_failure(
    ...
    *,
    ...
    error_detail = error[:_kb._EVENT_PAYLOAD_DETAIL_MAX]
    with _kb.write_txn(conn):
        row = conn.execute(
        ...
        if not (force_trip or failures >= effective_limit):
            if release_claim:
                ...
                conn.execute(
                    ...
                    "WHERE id = ? AND status = 'running'",
                    (retry_status, failures, error_preview, task_id),
                )
            else:
                conn.execute(
                    "UPDATE tasks SET consecutive_failures = ?, "
                    "last_failure_error = ? WHERE id = ?",
                    (failures, error_preview, task_id),
                )
            # Timeout/crash path's caller already emitted its own event.
            if end_run:
                run_id = _kb._end_run(
                    conn, task_id, outcome=outcome, status=outcome, error=error_detail,
                    metadata={"failures": failures, "retry_status": retry_status},
                )
                _kb._append_event(
                    conn, task_id, outcome,
                    {"error": error_detail, "failures": failures, "retry_status": retry_status},
                    run_id=run_id,
                )
            return False
        ...
        if end_run:
            ...
            run_id = _kb._end_run(
                conn, task_id, outcome="gave_up", status="gave_up", error=error_detail,
                metadata={
                    "failures": failures,
                    "trigger_outcome": outcome,
                    "effective_limit": effective_limit,
...
def check_respawn_guard(
    ...
    # A pr-babysitter card already targets an existing PR. Its progress comments
...
def _resolve_worker_terminal_config(
    ...
    try:
        ...
        try:
            ...
            profile_backend_explicit = isinstance(raw_terminal_cfg, dict) and "backend" in raw_terminal_cfg
        finally:
            reset_hermes_home_override(token)
        ...
        return (
            ...
            bool(mount_cwd),
            probe_env.get("TERMINAL_CWD") or None,
            profile_backend_explicit,
        )
    except Exception as exc:
        logging.getLogger(__name__).warning(
            "kanban worker: could not resolve terminal backend for HERMES_HOME=%r (%s); not pinning the host workspace",
            hermes_home,
            exc,
        )
        return None, False, None, False
...
def _remote_worker_spawn_dir(board: Optional[str] = None) -> Path:
    """Return a private host cwd that cannot leak dispatcher context to a remote worker."""
    path = _kb.worker_logs_dir(board=board).parent / "remote-cwd"
    try:
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    # Root terminal config is mirrored into the dispatcher's process env. Clear
    # those derived values so the assignee profile can resolve its own backend.
    try:
        from hermes_cli.config import TERMINAL_CONFIG_ENV_MAP, read_raw_config

        if isinstance(read_raw_config().get("terminal"), dict):
            worker_pinned = {"TERMINAL_CWD", "TERMINAL_TIMEOUT"}
            for env_var in TERMINAL_CONFIG_ENV_MAP.values():
                if env_var not in worker_pinned:
    ...
    if remote_worker:
        ...
        elif worker_backend_explicit or not inherited_cwd or os.path.isdir(inherited_cwd):
    ...
    # Goal-loop mode may be explicit on the task or inherited from a configured
    # checkable lane. Non-goal lanes can instead raise their single-run cap.
    kanban_cfg = _kb._load_kanban_cfg()
    effective_goal_mode, effective_goal_max_turns = _kb._resolve_lane_goal_defaults(
        task, kanban_cfg,
    )
    if effective_goal_mode:
        env["HERMES_KANBAN_GOAL_MODE"] = "1"
        if effective_goal_max_turns is not None:
            env["HERMES_KANBAN_GOAL_MAX_TURNS"] = str(effective_goal_max_turns)
        env.pop("HERMES_MAX_ITERATIONS", None)
    else:
        worker_max_iterations = _kb._resolve_worker_max_iterations(False, kanban_cfg)
        if worker_max_iterations is not None:
            env["HERMES_MAX_ITERATIONS"] = str(worker_max_iterations)
        else:
            env.pop("HERMES_MAX_ITERATIONS", None)
    ...
    # The worker switches HERMES_HOME to its assignee profile.  Pin the
    # dispatching profile's resolved safety threshold so it cannot silently
    # read a different profile's fd_headroom setting after the switch.
    env["HERMES_KANBAN_FD_HEADROOM"] = str(_kb._resolve_fd_headroom())
