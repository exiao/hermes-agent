...
class ModelSwitchResult:
    ...
    api_key: str = ""
    base_url: str = ""
    # True when the endpoint belongs to providers.<name>, not model.base_url.
...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    # An alias URL is an explicit route and must be persisted as model.base_url,
    # even if the preceding runtime came from providers.<name>.
    st.base_url_from_provider_config = False
    ...
    if alias_key:
        ...
        try:
            ...
            alias_runtime = {}
        # The alias endpoint is authoritative. Runtime resolution here is only
        # a credential lookup; letting its provider-config URL win loses the
        # alias route and can send the turn back to the previous proxy.
        st.base_url = da.base_url
...
class _Switch:
    ...
    def resolve_runtime(self, **kwargs) -> None:
        ...
        self.base_url_from_provider_config = bool(rt.get("base_url_from_provider_config"))
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    return ModelSwitchResult(
        ...
        base_url_from_provider_config=st.base_url_from_provider_config, api_mode=st.api_mode,
