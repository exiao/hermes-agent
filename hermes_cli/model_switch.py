...
def switch_model(
    ...
    if provider_changed or explicit_provider:
        ...
        if keep_current_ollama_endpoint:
            ...
            try:
                ...
                # provider like "ollama-launch" that resolve_runtime_provider
                # doesn't know), keep existing credentials. Otherwise use the
                # resolved values (credential rotation, base_url adjustments).
                api_key = runtime.get("api_key", "")
                base_url = runtime.get("base_url", "")
                base_url_from_provider_config = bool(
                    runtime.get("base_url_from_provider_config")
    ...
    if resolved_alias:
        ...
        if _da is not None and _da.base_url:
            ...
            # FORK: the alias endpoint is now the source of truth for
            # base_url, NOT the inherited providers.<provider>.base_url.
            # Reset provenance so the CLI/TUI persistence paths write
            # model.base_url (the alias URL) instead of dropping it —
            # otherwise `/model <alias> --global` saves the model but loses
            # the endpoint, and after restart the model runs against
            # providers.<provider>.base_url (e.g. the loopback proxy).
            base_url_from_provider_config = False
