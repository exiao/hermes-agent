...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def _run_hook_callback_bounded(
        ...
        """Run a bounded callback while allowing healthy concurrent invocations."""
        import contextvars
        import time
        from hermes_cli.plugins_dispatch import _HOOK_SKIPPED
        ...
        with self._hook_timeout_lock:
            ...
            stuck = bool(in_flight) and any(
                now - started >= timeout for started in in_flight.values())
            if (suppressed_until is not None and suppressed_until > now) or stuck:
                logger.warning(
            ...
            self._hook_running_callbacks.setdefault(callback_key, {})[token] = now
        ...
        def _runner() -> None:
            try:
                ...
                with self._hook_timeout_lock:
                    entries = self._hook_running_callbacks.get(callback_key)
        ...
        threading.Thread(
            target=_runner, name=f"hermes-hook-{callback_name}"[:40], daemon=True).start()
