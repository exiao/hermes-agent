...
from hermes_cli.models_local import (
    ...
    fetch_ollama_cloud_models as _fetch_ollama_cloud_models)

logger = logging.getLogger(__name__)


def fetch_ollama_cloud_models(
    api_key: str = "", base_url: str = "", *, force_refresh: bool = False
) -> list[str]:
    """Scope-aware wrapper around the decomposed Ollama Cloud catalog loader."""
    from agent.secret_scope import get_secret
    api_key = api_key or (get_secret("OLLAMA_API_KEY", "") or "")
    base_url = base_url or (get_secret("OLLAMA_BASE_URL", "") or "")
    return _fetch_ollama_cloud_models(
        api_key=api_key, base_url=base_url, force_refresh=force_refresh)
...
def _openai_discovery_base_url(provider: str) -> str:
    ...
    from agent.secret_scope import get_secret
    env_raw = (get_secret("OPENAI_BASE_URL", "") or "").strip().rstrip("/")
    if env_raw:
        return env_raw
    try:
    ...
    try:
        ...
        if pcfg is not None:
            for ev in getattr(pcfg, "api_key_env_vars", ()) or ():
                parts.append(f"{ev}={get_secret(ev, '') or ''}")
            bev = getattr(pcfg, "base_url_env_var", "") or ""
            if bev:
                parts.append(f"{bev}={get_secret(bev, '') or ''}")
    except Exception:
        pass
...
def _is_named_profile_scope() -> bool:
    """Whether the active home is a named ``<root>/profiles/<name>`` profile."""
    try:
        from hermes_constants import get_hermes_home
        return Path(os.path.abspath(get_hermes_home())).parent.name == "profiles"
    except Exception:
        return False


def _scoped_anthropic_pool_token() -> str:
    """Read a usable Anthropic token from the active profile auth store only."""
    try:
        from agent.credential_pool import PooledCredential, STATUS_DEAD, _exhausted_until
...
def _fetch_anthropic_models(
    ...
    # During the temporary decomposition compatibility window, callers may
    # have patched the old adapter-owned resolver. Honor an explicit module
    # attribute without resolving the deprecated __getattr__ path ourselves.
    adapter = sys.modules.get("agent.anthropic_adapter")
    token_resolver = (
        adapter.__dict__.get("resolve_anthropic_token", resolve_anthropic_token)
        if adapter is not None else resolve_anthropic_token)
    ...
    if not explicit_key and active_scope is not None:
        for name in ("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN", "ANTHROPIC_API_KEY"):
            scoped_token = str(get_secret(name, "") or "").strip()
            if scoped_token:
                break
        if not scoped_token:
            scoped_token = _scoped_anthropic_pool_token()
        # The default profile owns the process Claude identity; named profiles
        # must never borrow it when their scoped sources miss.
        if not scoped_token and not _is_named_profile_scope():
            scoped_token = (token_resolver() or "").strip()
    token = explicit_key or (
        scoped_token if active_scope is not None else token_resolver())
    if not token:
