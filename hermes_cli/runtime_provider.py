...
def _anthropic_base_url_override_ok(base_url: str, *, allow_loopback: bool = False) -> bool:
    ...
    return bool(hostname) and (hostname == "api.anthropic.com" or hostname.endswith((".anthropic.com", ".claude.com", ".azure.com"))
                               ...
                               or (allow_loopback and _loopback_hostname(hostname)))
...
def _anthropic_cfg_base_url(model_cfg: Dict[str, Any]) -> str:
    ...
    return cfg_base_url if _anthropic_base_url_override_ok(
        cfg_base_url, allow_loopback=bool(model_cfg.get("_base_url_from_provider_config"))) else ""
...
def _get_model_config() -> Dict[str, Any]:
    ...
    provider_cfg = providers_cfg.get(configured_provider) if (
        configured_provider and isinstance(providers_cfg, dict)) else None
    if isinstance(provider_cfg, dict):
        if not str(cfg.get("base_url") or "").strip():
            provider_url = provider_cfg.get("base_url") or provider_cfg.get("api") or provider_cfg.get("url")
            if provider_url:
                cfg["base_url"] = str(provider_url).strip()
                cfg["_base_url_from_provider_config"] = True
        for target, candidates in {
            "api_key": ("api_key",), "key_env": ("key_env",),
            "api_mode": ("transport", "api_mode")}.items():
            if str(cfg.get(target) or "").strip():
                continue
            value = next((provider_cfg.get(name) for name in candidates if provider_cfg.get(name)), None)
            if value:
                cfg[target] = str(value).strip()
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider == "anthropic":
        return "anthropic_messages", (_anthropic_cfg_base_url(model_cfg) or base_url
                                      or _getenv("ANTHROPIC_BASE_URL", "").strip().rstrip("/")
...
def _resolve_runtime_from_pool_entry(*, provider: str, entry: PooledCredential, requested_provider: str,
    ...
    return _runtime(
        provider, api_mode, base_url, _pool_entry_api_key(entry),
        source=getattr(entry, "source", "pool"), credential_pool=pool,
        requested_provider=requested_provider,
        base_url_from_provider_config=(
            provider == "anthropic" and bool(model_cfg.get("_base_url_from_provider_config"))))
...
def _explicit_anthropic(requested_provider, model_cfg, api_key, base_url, target_model):
    from_provider_config = not bool(base_url) and bool(model_cfg.get("_base_url_from_provider_config"))
    ...
    return _runtime(
        "anthropic", "anthropic_messages", base_url, api_key, source="explicit",
        requested_provider=requested_provider,
        base_url_from_provider_config=from_provider_config)
...
def _anthropic_env_runtime(requested_provider: str, model_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    base_url = (_anthropic_cfg_base_url(model_cfg)
                or _getenv("ANTHROPIC_BASE_URL", "").strip().rstrip("/")
                or _ANTHROPIC_DEFAULT_BASE_URL)
    ...
    return _runtime(
        "anthropic", "anthropic_messages", base_url, token, source="env",
        requested_provider=requested_provider,
        base_url_from_provider_config=bool(model_cfg.get("_base_url_from_provider_config")))
