...
# Extension-anchored MEDIA matcher built from the shared delivery extension sets.
from gateway.platforms.base import MEDIA_DELIVERY_EXTS as _MEDIA_DELIVERY_EXTS

_MEDIA_TAG_EXTRA_EXTS = (
    ".toml", ".ini", ".cfg", ".conf",
    ".py", ".js", ".ts", ".sh", ".rb", ".go",
    ".rs", ".java", ".c", ".cpp", ".h", ".hpp", ".sql",
    ".css", ".log",
)
            ...
            if isinstance(payload, dict) and payload.get("success"):
                for field in _JSON_MEDIA_TOOL_PATH_FIELDS:
                    ...
                    if (isinstance(path, str)
                            ...
                            and _media_path_dedup_key(path) not in history_media_path_keys):
...
def _collect_history_media_paths(agent_history: List[Dict[str, Any]]) -> set:
    ...
    # Model-generated MEDIA: tags in ASSISTANT text (#53586). Without this a
    # model that echoes a previous tag re-delivers the same file every turn.
    for msg in agent_history:
        if msg.get("role") != "assistant":
            continue
        content = str(msg.get("content") or "")
    ...
    for msg in agent_history:
        ...
        if "MEDIA:" in content:
            ...
            continue
        # JSON-payload tools (image_generate) carry the path in the result
        # JSON with no MEDIA: tag. Missing this caused #46627: after a
        # compression boundary the auto-append fallback rescans history,
        # re-discovers the path absent from the dedup set, and re-emits the
        # MEDIA tag every turn.
...
from gateway.config import (
    ChannelOverride, Platform, GatewayConfig, PlatformConfig, PORT_BINDING_PLATFORM_VALUES,
...
def _install_runner_ref(runner):
    """Temporarily install a runner weakref (used by heartbeat tests and diagnostics)."""
    global _gateway_runner_ref
    previous = _gateway_runner_ref
    _gateway_runner_ref = _weakref.ref(runner)
    try:
        yield runner
    finally:
        _gateway_runner_ref = previous


def _heartbeat_active_agents() -> None:
    """Best-effort status heartbeat using the live aggregate work count."""
    try:
        runner = _gateway_runner_ref()
        if runner is not None:
            runner._persist_active_agents()
    except Exception as exc:
        logger.debug("active_agents heartbeat error: %s", exc)
...
def _quoted_media_path_valid_on_backend(host_path: str) -> bool:
    """Whether an untranslated quoted-media path is reachable by the active backend."""
    if os.environ.get("TERMINAL_ENV", "local") in ("local", "docker"):
        return True
    try:
        from tools.environments.file_sync import iter_sync_files

        return any(host == host_path and remote == host_path for host, remote in iter_sync_files())
    except Exception:
        return False


def format_reply_pointer(event, message_text: str) -> str:
    """Prefix a quote reply with the exact text/media pointer the user selected."""
    ...
    for path in getattr(event, "reply_to_media_paths", None) or []:
        ...
        if visible != path or _quoted_media_path_valid_on_backend(path):
            media_paths.append(visible)
    if media_summary := getattr(event, "reply_to_media_summary", None):
...
_upstream_send_progress_messages = TurnRunner.send_progress_messages


async def _send_progress_messages_with_separate_fallback(self):
    """Preserve one-message-per-tool progress, including cancellation drains."""
    import queue as _queue
    from types import SimpleNamespace

    ctx = self._ctx
    if ctx.progress_grouping != "separate":
        return await _upstream_send_progress_messages(self)
    adapter = self._runner._adapter_for_source(ctx.source) if ctx.progress_queue else None
    if not adapter:
        return
    state = SimpleNamespace(adapter=adapter, progress_lines=[], progress_msg_id=None, can_edit=False)
    pending = None
    last_send = 0.0
    ...
    async def deliver(raw) -> bool:
        nonlocal last_send
        if self._is_reset_marker(raw):
            self._reset_progress_bubble(state)
            await asyncio.sleep(0)
            return True
        remaining = 1.5 - (time.monotonic() - last_send)
        if remaining > 0:
            await asyncio.sleep(remaining)
        if not ctx._run_still_current() or self._agent_interrupted():
            discard()
            return False
        msg = self._progress_absorb(state, raw)
        await self._send_progress_text(state, msg)
    ...
    try:
        while True:
            ...
            try:
                ...
                continue
            if not await deliver(pending):
                return
            pending = None
    except asyncio.CancelledError:
        if self._agent_interrupted() or not ctx._run_still_current():
            discard()
            return
        while pending is not None or not ctx.progress_queue.empty():
            raw = pending
            pending = None
            if raw is None:
                try:
                    ...
                    break
            if not await deliver(raw):
                return


TurnRunner.send_progress_messages = _send_progress_messages_with_separate_fallback

_upstream_progress_build_message = TurnRunner._progress_build_message


def _progress_build_message_with_display_rules(self, tool_name, preview, args):
    """Apply fork display aliases/rewrite rules and customer-channel code hiding."""
    from gateway.display_config import resolve_display_setting

    ctx = self._ctx
    user_config = ctx.user_config if isinstance(ctx.user_config, dict) else {}
    display = user_config.get("display") or {}
    if not isinstance(display, dict):
        display = {}
    tool_display = display.get("tool_display")
    if not isinstance(tool_display, dict):
        tool_display = {}
    legacy_display = display.get("tool_friendly_names")
    if not tool_display and isinstance(legacy_display, dict):
        tool_display = legacy_display
    legacy_show = display.get("tool_show_preview")
    legacy_show = set(legacy_show) if isinstance(legacy_show, list) else set()

    command_text = preview or ""
    if not command_text and isinstance(args, dict):
        command_text = args.get("command", args.get("code", "")) or ""
    resolved = None
    rewrites = display.get("tool_display_rewrite")
    if isinstance(rewrites, dict):
        for pattern, replacement in rewrites.items():
            try:
                match = re.search(pattern, str(command_text))
                if match:
                    repl = re.sub(r"\$(\d+)", r"\\\1", str(replacement))
                    resolved = re.sub(pattern, repl, match.group(0))
                    break
            except re.error:
                continue
    if resolved is None and tool_name in tool_display:
        template = str(tool_display[tool_name])
        if "{preview}" in template:
            resolved = template.replace("{preview}", preview or "")
        elif tool_name in legacy_show and preview:
            resolved = f'{template}: "{preview}"'
        else:
            resolved = template + "..."

    platform_key = _platform_config_key(ctx.source.platform) if ctx.source is not None else ""
    hide_code = bool(resolve_display_setting(
        user_config, platform_key, "hide_code_in_progress", False
    ))
    if resolved is not None or (hide_code and tool_name in {"terminal", "execute_code", "process"}):
        from agent.display import get_tool_emoji

        emoji = get_tool_emoji(tool_name, default="⚙️")
        ctx.last_was_terminal_block[0] = False
        return f"{emoji} {resolved}" if resolved else f"{emoji} {tool_name}..."
    return _upstream_progress_build_message(self, tool_name, preview, args)


TurnRunner._progress_build_message = _progress_build_message_with_display_rules
...
class GatewayRunner(
    ...
    @staticmethod
    def _prepend_inbound_reply_context(event: MessageEvent, source: SessionSource,
                                       message_text: str) -> str:
        """Preserve upstream's Discord trigger hint, then add the fork's full reply pointer."""
        if (
            source is not None
            ...
            if _discord_tools_loaded():
                message_text = (
                    f"[Triggering message id: `{event.message_id}` — use as `message_id` for "
                    f"reply/react/pin via the discord tools.]\n\n{message_text}"
                )
        return format_reply_pointer(event, message_text)

    async def _prepare_busy_steer_text(self, event: MessageEvent) -> str:
        """Transcribe a busy voice steer and retain its native quote pointer."""
        ...
        body = (enriched_text or text).strip() if successful_transcripts else text
        return format_reply_pointer(event, body) if body else ""

    def _adapter_for_source(self, source: Optional[SessionSource]):
        """Resolve relay provenance before native transport provenance, then fail closed by profile."""
        if source is None:
            return None
        # A retained transport identity is stronger than a reconstructed
        # profile stamp (and may itself be the Relay adapter).
        ...
        profile = str(getattr(source, "profile", None) or "").strip() or None
        # Port-binding listeners are default-owned and serve routed profiles
        # through one shared adapter; they are the intentional exception to
        # the secondary-profile fail-closed rule.
        if getattr(platform, "value", platform) in PORT_BINDING_PLATFORM_VALUES:
        ...
        if profile:
            ...
            # Prefer upstream's renamed launch-time identity, but retain the
            # fork's old attribute for partial runners and integrations built
            # before the decomposition.  Never consult a scoped live identity
            # when either captured value exists.
            primary_profile = (
                getattr(self, "_primary_profile_name", None)
                or getattr(self, "_owner_profile_name", None)
            )
            if primary_profile is None:
                active_profile = getattr(self, "_active_profile_name", None)
                primary_profile = active_profile() if callable(active_profile) else "default"
            if profile != primary_profile:
                return None
        return self._primary_adapters().get(platform)

    async def _load_transcript_for_source(self, session_id: str, source: SessionSource) -> list:
        """Read a routed conversation from the same profile store that owns its writes."""
    ...
    async def _hmwa_prepare_turn(self, event, source, session_entry, session_key,
                                 quick_key, run_generation):
        """Run upstream turn preparation under the routed profile's DB and secret scope."""
        if not getattr(getattr(self, "config", None), "multiplex_profiles", False):
            return await GatewayTurnMixin._hmwa_prepare_turn(
                self, event, source, session_entry, session_key, quick_key, run_generation
            )
        async with _async_profile_runtime_scope(self._resolve_profile_home_for_source(source)):
            return await GatewayTurnMixin._hmwa_prepare_turn(
                self, event, source, session_entry, session_key, quick_key, run_generation
    ...
    def _command_handler_table(self, names) -> Dict[str, Any]:
        """Scope decomposed slash handlers to the routed profile without copying their bodies."""
        table = GatewayBusySessionMixin._command_handler_table(self, names)
        if not getattr(getattr(self, "config", None), "multiplex_profiles", False):
            return table
        ...
        return {name: scoped(handler) for name, handler in table.items()}

    def _build_stream_consumer_config(
        self, source: SessionSource, scfg: Any, adapter: Any, *, on_missing_cursor: str,
    ):
        """Add paragraph-buffered streaming for adapters that cannot edit sent messages."""
        config, pause = GatewayTurnMixin._build_stream_consumer_config(
            self, source, scfg, adapter, on_missing_cursor=on_missing_cursor
        )
        supports_edit = getattr(adapter, "SUPPORTS_MESSAGE_EDITING", True)
        supports_native = bool(getattr(adapter, "SUPPORTS_NATIVE_STREAMING", False))
        config.no_edit_mode = not supports_edit and not supports_native
        return config, pause
...
def _start_gateway_housekeeping(
    ...
    chores += [
        # Keep gateway_state.json's active count and mtime live even when only cron/API work changes it.
        # The Signal watchdog uses this as a liveness heartbeat; a stale non-zero count can otherwise
        # trigger a false kill after the last cron run finishes.
        (1, "Active-agent status heartbeat", _heartbeat_active_agents),
