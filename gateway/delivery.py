...
def resolve_delivery_transport(platform: Platform, config: GatewayConfig,
                               ...
                               allow_disabled_native: bool = False) -> Optional[DeliveryTransport]:
    ...
    authoritative_platforms = getattr(adapters, "allow_disabled_native_platforms", ())
    native_is_authoritative = allow_disabled_native or platform in authoritative_platforms
    if native is not None and (
        native_config is None or native_config.enabled or native_is_authoritative
    ):
        # Downstream delivery code re-checks ``pconfig.enabled``. A live adapter with no config
        # block, or a deliberately fronted disabled adapter, has already passed the resolver's
        # transport guard and needs an enabled runtime config for that second gate.
        runtime_config = native_config
        if runtime_config is None:
            runtime_config = PlatformConfig(enabled=True)
        elif not runtime_config.enabled:
            runtime_config = replace(runtime_config, enabled=True)
        return DeliveryTransport(native, runtime_config, platform)
...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: Dict[Platform, Any] = None,
                 ...
                 allow_disabled_native: bool = False):  # profile-local registry when omitted
        self.config = config
