...
def _runtime_status_write_transaction(path: Path):
    """Serialize runtime-status read/modify/write across threads and processes."""
    ...
    with _runtime_status_write_lock:
        path.parent.mkdir(parents=True, exist_ok=True)
        handle = open(path.with_name(_RUNTIME_STATUS_LOCK_FILE), "a+", encoding="utf-8")
        try:
            if _IS_WINDOWS:
                handle.seek(0, os.SEEK_END)
                if handle.tell() == 0:
    ...
    *, gateway_state: Any = _UNSET, exit_reason: Any = _UNSET, restart_requested: Any = _UNSET,
    ...
    with _runtime_status_write_transaction(path):
        _write_runtime_status_unlocked(
            ...
            restart_requested=restart_requested, active_agents=active_agents,
            ...
            session_store=session_store, clear_profile_platforms=clear_profile_platforms,
        )


def write_runtime_status_desired_state(path: Path, desired_state: str) -> None:
    """Persist supervisor intent without stamping the command process as the gateway owner."""
    with _runtime_status_write_transaction(path):
