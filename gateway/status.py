...
def _write_runtime_status_unlocked(
    *,
    ...
    # ``current_record`` is always bound (later per-platform writer-identity
    # stamping reads it), but the fork's durable desired-state path must NOT
    # overwrite the top-level PID metadata when refresh_pid_metadata=False —
    # a lifecycle CLI command writing desired-state would otherwise stamp the
    # record with the CLI's own PID and make a live gateway look dead.
