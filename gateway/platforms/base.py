    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                # No delivery-time dedup against history: a MEDIA: tag records
                # what the model asked to send, never what the platform
                # accepted. Suppressing on it silently drops the attachment the
                # user asked for (and makes a failed upload unretryable). A
                # duplicate image is the cheaper failure; always deliver.
