...
class MessageEvent:
    ...
    # Human-readable metadata for textless quoted media, plus any safe local copies the adapter can resolve.
...
class SendResult:
    ...
    # A non-empty prefix of a split payload reached the platform. Retrying the
    # whole payload would duplicate that prefix, so the shared retry layer must
...
class BasePlatformAdapter(ABC):
    ...
    def _span_is_only_real_media_tag(span_text: str) -> bool:
        """Whether a protected span contains only validated MEDIA directives."""
        ...
        if text.startswith("```"):
            ...
            inner = body[:-3] if body.endswith("```") else body
        else:
            inner = text.strip("`")
        lines = []
        ...
        if not lines:
            return False
        for line in lines:
            match = MEDIA_TAG_CLEANUP_RE.fullmatch(line.strip("`").strip())
            if match is None:
                return False
            path = _normalize_media_tag_path(match.group("path"))
            try:
    ...
    @staticmethod
    def _classify_protected_spans(content: str) -> Tuple[list, list]:
        """Return ``(masked, delivered)`` protected wrapper spans.

        A wrapper containing only real, allowlisted MEDIA tags stays scannable;
        examples or mixed prose remain masked. Fences own nested inline spans.
        """
        masked: list = []
        delivered: list = []
        fence_ranges: list = []

        def inside_fence(start: int, end: int) -> bool:
            return any(fs <= start and end <= fe for fs, fe in fence_ranges)

        for match in _FENCED_CODE_RE.finditer(content):
            span = match.span()
            fence_ranges.append(span)
            (delivered if BasePlatformAdapter._span_is_only_real_media_tag(match.group(0)) else masked).append(span)
        for match in _INLINE_CODE_RE.finditer(content):
            ...
            if re.search(r"MEDIA:\s*$", content[max(0, start - 20):start]):
                continue
            (delivered if BasePlatformAdapter._span_is_only_real_media_tag(match.group(0)) else masked).append((start, end))
    ...
    def _delivered_media_wrapper_spans(content: str) -> list:
        return BasePlatformAdapter._classify_protected_spans(content)[1]
    ...
    def _mask_protected_spans(content: str) -> str:
        ...
        valid) against MEDIA false positives. Wrappers made only of validated
        MEDIA tags remain scannable so deliberate attachments still deliver."""
        masked, _delivered = BasePlatformAdapter._classify_protected_spans(content)
        return _blank_spans(content, masked)
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        for _, safe_path, _ in _extensionless_media_matches(scan_content):
            _add(safe_path)
        # Remove the delivered tag and its otherwise-empty wrapper. Protected
        # examples survive verbatim.
        if media:
    ...
    def _strip_delivered_media_text(text: str) -> str:
        """Delete deliverable MEDIA tags and wrappers while preserving examples."""
        if not text or not _has_media_directives(text):
            return text
        cleaned = text.replace("[[audio_as_voice]]", "").replace("[[as_document]]", "")
        masked = _mask_media_scan_text(cleaned)
        tag_spans = _real_media_tag_spans(masked)
        if not tag_spans:
            return cleaned
        wrapper_spans = [
            (start, end)
            for start, end in BasePlatformAdapter._delivered_media_wrapper_spans(cleaned)
            if any(start <= tag_start and tag_end <= end for tag_start, tag_end in tag_spans)
        ]
        return re.sub(r"\n{3,}", "\n\n", _delete_spans(cleaned, tag_spans + wrapper_spans))
    ...
    def strip_media_directives_for_display(text: str) -> str:
        ...
        return BasePlatformAdapter._strip_delivered_media_text(text).rstrip()
    ...
    def _is_retryable_error(error: Optional[str]) -> bool:
        ...
        # Explicit 4xx rejections are permanent even when a provider's exception package contains
        # the word "network" (Signal's response-code exception does).
    ...
    def _is_ambiguous_delivery_error(error: Optional[str]) -> bool:
        """True for HTTP 5xx sends that may have succeeded before the response failed."""
        if not error:
            return False
        return bool(re.search(
            r"\[5\d\d\]|(?:bad response:\s*|http\s+)5\d\d|error\s+'5\d\d",
            error.lower(),
        ))
    ...
    def build_source(
        ...
        # but must not move a credential-owned event into another profile.
        owner_profile = getattr(self, "_owner_profile", None)
        profile = owner_profile.strip() if isinstance(owner_profile, str) and owner_profile.strip() else None
        profile_route_rejected = False
        if profile is None and self.gateway_runner is not None:
