...
class BasePlatformAdapter(ABC):
    ...
    def _strip_delivered_media_text(text: str) -> str:
        ...
        *locate* real, deliverable tag spans, deletes those, and also removes any
        wrapper whose entire content was deliverable tag(s). Protected example
        spans survive verbatim. Shared by ``extract_media`` and the streaming
        display path.
        """
        if (
            "MEDIA:" not in text
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                # A MEDIA: tag records what the model asked to send, never what
                # the platform accepted, so suppressing on it would silently
                # drop the attachment the user asked for. The auto-append path
                # in GatewayRunner._run_agent_inner already deduplicates
                # auto-appended tags via _collect_auto_append_media_tags with
                # history_media_paths. Bare-file-path dedup still applies to
                # local_files below via the same _history_media_paths set.
