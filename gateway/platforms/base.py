...
class BasePlatformAdapter(ABC):
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        media_pattern = re.compile(
            r'''[`"']?MEDIA:\s*(?P<path>`[^`\n]+`|"[^"\n]+"|'[^'\n]+'|(?:~/|/)\S+(?:[^\S\n]+\S+)*?\.(?:png|jpe?g|gif|webp|mp4|mov|avi|mkv|webm|ogg|opus|mp3|wav|m4a|epub|pdf|zip|rar|7z|tar|gz|docx?|xlsx?|pptx?|txt|md|yaml|yml|json|csv|py|js|ts|sh|log|xml|html|css|rtf|toml|cfg|ini|conf|sql|rb|go|rs|java|c|cpp|h|hpp|apk|ipa)(?=[\s`"',;:)\]}]|$)|\S+)[`"']?'''
        )
