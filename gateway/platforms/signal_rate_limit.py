...
def _signal_send_timeout(num_attachments: int, total_attachment_bytes: int = 0) -> Any:
    """Bound local connection/write setup, but never deadline Signal's remote upload read.

    signal-cli receives file paths and uploads the attachments before answering the local RPC. A scalar
    timeout therefore imposes an arbitrary minimum uplink speed and reports phantom failures for large
    files even when signal-cli completes the send moments later.
    """
