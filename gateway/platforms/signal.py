...
SSE_RECONNECT_ESCALATE_AFTER = 2
SSE_DEAD_STREAM_ESCALATE_AFTER = 2
...
def _render_mentions(text: str, mentions: list, *, bot_account: str = "",
                     sender_name: str = "") -> str:
    """Replace Signal mention placeholders without exposing phone numbers to the model."""
    ...
    for mention in sorted(mentions, key=lambda m: m.get("start", 0), reverse=True):
        ...
        number_norm = str(mention.get("number") or "").strip().lstrip("+")
        display = "assistant" if bot_norm and number_norm == bot_norm else "member"
        text = text[:start] + f"@{display}" + text[start + length:]
    return text
...
class SignalAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Keep inbound SSE on a dedicated pool. Closing a stalled streaming response on the RPC
        # pool could otherwise wait for unrelated outbound traffic before the listener reconnects.
    ...
    async def _sse_listener(self, listener_generation: Optional[int] = None) -> None:
        ...
        while self._running and listener_generation == self._sse_listener_generation:
            try:
                ...
                async with self.sse_client.stream("GET", url, headers={"Accept": "text/event-stream"},
                    ...
                    if gap > HEALTH_CHECK_STALE_THRESHOLD:
                        logger.warning(
                            "Signal SSE: reconnected after %.0fs without events — any messages "
                            "delivered in that window were dropped (the daemon does not replay them)",
                            gap,
                        )
                    else:
                    ...
                    async for chunk in response.aiter_text():
                        if not self._running or listener_generation != self._sse_listener_generation:
                            break
                        buffer += chunk
                        while "\n" in buffer:
        ...
        while self._running:
            ...
            if self._reconnect_requested_at_generation == self._sse_generation:
                ...
                if self._stale_checks_since_reconnect >= SSE_RECONNECT_ESCALATE_AFTER:
                    logger.error(
                        "Signal: SSE still idle %.0fs after %d reconnect checks — recreating listener task",
                        elapsed, self._stale_checks_since_reconnect,
                    )
                    await self._restart_sse_listener()
                else:
                    logger.warning("Signal: SSE idle %.0fs, reconnect pending", elapsed)
                continue
            if self._reconnects_without_events >= SSE_DEAD_STREAM_ESCALATE_AFTER:
                logger.error(
                    "Signal: SSE idle %.0fs after %d silent reconnects — recreating listener task",
    ...
    def _request_reconnect(self) -> None:
        """Request a reconnect and record whether the request actually lands."""
    ...
    async def _restart_sse_listener(self) -> None:
        """Cancel and rebuild a listener whose requested reconnect did not restore traffic."""
        if not self._running:
            return
        old_task, old_client = self._sse_task, self.sse_client
        self._sse_listener_generation += 1
        self._sse_response = None
        ...
        self._stale_checks_since_reconnect = 0
        self._reconnects_without_events = 0
        self._sse_task = asyncio.create_task(self._sse_listener(self._sse_listener_generation))
    ...
    async def _collect_attachments(self, attachments_data: list) -> Tuple[List[str], List[str]]:
        ...
        for att in attachments_data:
            ...
            try:
                ...
                cached_path, ext = await self._fetch_attachment(att_id, filename)
                if cached_path:
    ...
    async def _handle_envelope(self, envelope: dict) -> None:
        ...
        if text and (mentions := data_message.get("mentions", [])):
            text = _render_mentions(text, mentions, bot_account=self.account, sender_name=sender_name)
        if is_group:
    ...
    def _remember_recipient_identifiers(self, number: Optional[str], service_id: Optional[str]) -> None:
        ...
        if number == service_id:
            return  # UUID-only envelopes must not overwrite an earlier number↔UUID alias.
    ...
    def _remember_sent_attachments(self, timestamp: Any, chat_id: str, paths: List[str]) -> None:
        """Map an outbound timestamp and destination to local attachment paths."""
    ...
    def _describe_quoted_attachments(quote_data: Any) -> Optional[str]:
        """Return a human-readable description of Signal quote attachment metadata."""
        attachments = quote_data.get("attachments") if isinstance(quote_data, dict) else None
        ...
        for attachment in attachments:
            ...
            filename = str(attachment.get("filename") or attachment.get("fileName") or "").strip()
            noun = (
                "an image" if content_type.startswith("image/")
                else "a video" if content_type.startswith("video/")
                else "an audio file" if content_type.startswith("audio/")
                else "a file"
            )
            details = [value for value in (content_type, filename) if value]
            descriptions.append(f"{noun} ({', '.join(details)})" if details else noun)
        if not descriptions:
            return None
        return descriptions[0] if len(descriptions) == 1 else (
            f"{len(descriptions)} attachments: " + "; ".join(descriptions)
        )
    ...
    def _conversation_key_aliases(self, chat_id: str) -> List[str]:
        ...
        if str(chat_id).startswith("group:"):
            return aliases
        for alias in (
            self._recipient_uuid_by_number.get(str(chat_id)),
            self._recipient_number_by_uuid.get(str(chat_id)),
        ):
    ...
    def _resolve_quoted_media_paths(self, quote_id: Optional[str], chat_id: str) -> List[str]:
        ...
        for alias in self._conversation_key_aliases(chat_id):
            ...
            if cached:
                return [path for path in cached if path and Path(path).exists()]
        return []
    ...
    async def _rpc(self, method: str, params: dict, rpc_id: str = None, *, log_failures: bool = True,
                   raise_on_rate_limit: bool = False, raise_on_error: bool = False,
    ...
    @staticmethod
    def _partial_send_failure(sent_chunks: int, total_chunks: int, error: str,
                              raw_response=None) -> SendResult:
        """Prevent a full retry after a non-empty prefix was already delivered."""
        if not sent_chunks:
        ...
        logger.error(
            ...
            sent_chunks, total_chunks, error,
        )
        return SendResult(
            success=False,
    ...
    async def _send_attachment_batch(self, scheduler, params: Dict[str, Any], n: int, label: str,
        ...
        total_bytes = sum(
            Path(path).stat().st_size for path in attachment_paths if Path(path).exists()
        )
        send_timeout = _signal_send_timeout(n, total_bytes)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if success:
                self._track_sent_timestamp(result)
                if isinstance(result, dict) and chat_id:
                    self._remember_sent_attachments(
                        result.get("timestamp"), chat_id, list(attachment_paths)
                    )
    ...
    async def _send_file(self, chat_id: str, file_path: str, caption: Optional[str], fail_error: str) -> SendResult:
        ...
        try:
            ...
            return SendResult(success=False, error=f"Attachment file not found: {file_path}")
        result = await self._rpc(
            "send", params, timeout=_signal_send_timeout(1, file_size)
        )
        if result is None:
            return SendResult(success=False, error=fail_error)
        success, err_msg = self._validate_send_result(result)
        if not success:
