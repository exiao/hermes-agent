...
class GatewaySessionCommandsMixin:
    ...
    async def _cleanup_old_agent_for_reset(self, session_key: str) -> None:
        """Schedule old-agent teardown without delaying session rotation.

        Snapshot the transcript before eviction: the cache's soft-release clears the live list,
        but memory-provider session-end hooks still need the completed conversation.
        """
        ...
        if _old_agent is None:
            return
        messages_snapshot = _UNSET
        live_messages = getattr(_old_agent, "_session_messages", None)
        if isinstance(live_messages, list):
            messages_snapshot = list(live_messages)
        self._retain_background_task(asyncio.create_task(
            self._cleanup_agent_resources_off_loop(
                _old_agent,
                context=f"/new reset {session_key}",
                session_messages=messages_snapshot,
            )
        ))
    ...
    async def _run_manual_compression(self, source, session_entry, history: list, partial: bool,
        ...
        tmp_agent = await self._build_manual_compression_agent(
            session_entry.session_id, model, runtime_kwargs, source)
        try:
            ...
            persisted = await self._persist_manual_compression(
                tmp_agent, session_entry, source, compressed, partial=partial, tail=tail)
    ...
    async def _build_manual_compression_agent(
        self, session_id: str, model, runtime_kwargs: dict, source: SessionSource
    ):
    ...
    async def _persist_manual_compression(
        self, tmp_agent, session_entry, source, compressed, *, partial: bool, tail: list
    ) -> Optional[bool]:
        ...
        archive; an unchanged id without in-place means rotation FAILED. Returns true when a
        transcript was committed, false when persistence failed, and None for a summarizer abort."""
        ...
        elif getattr(tmp_agent, "_last_compaction_in_place", False):
            if partial and tail:
                db = getattr(tmp_agent, "_session_db", None) or self._session_db
                archive_and_compact = getattr(db, "archive_and_compact", None)
                if not callable(archive_and_compact):
                    raise RuntimeError(
                        f"failed to persist rejoined compression tail for session {new_session_id}")
                archive_and_compact(new_session_id, compressed)
        else:
            compressor = getattr(tmp_agent, "context_compressor", None)
            if getattr(compressor, "_last_compress_aborted", False):
                return None
            logger.warning(
                "Manual /compress: session rotation did not occur (session_id unchanged), in-place "
                "mode is off, and the compressor did not abort — nothing persisted; preserving the "
                "original transcript instead of overwriting it (#44794).")
            return False
