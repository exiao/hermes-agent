...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        def _read_current_config() -> None:
            ...
            nonlocal user_provs, custom_provs, persist_global, excluded_provs
            persist_global = resolve_persist_behavior(
                is_global_flag,
                is_session,
                is_once=one_turn,
        ...
        def _persist_switched_model(result) -> None:
            ...
            def _do_persist() -> None:
                ...
                if isinstance(raw_model, dict):
                    ...
                    model_cfg = {}
                    cfg["model"] = model_cfg
                # Upstream #25106 behavior: a model/base_url/provider change
                # invalidates a context_length pinned for the OLD route.
                # Evaluated BEFORE the new values overwrite model_cfg.
                try:
                ...
                # Named providers always resolve base_url/api_mode fresh, so any
                # leftover is cleared unconditionally below. Custom providers have
                # no registry entry to re-derive from, so they need an explicit
                # set-or-clear here — a lone `if result.base_url:` would leave a
                # stale base_url behind when switching to a custom provider whose
                # resolver returned an empty base_url (#25107).
        ...
        try:
            ...
            tmp_agent = AIAgent(
                **runtime_kwargs,
                ...
                # Fork patch: pass the full gateway source identity so the
                # continuation session row _compress_context creates records the
                # real conversation (user/chat/thread) instead of only platform.
                # Without these, post-compression messages fall out of
                # per-user/per-chat SessionDB lookups.
                user_id=source.user_id,
                chat_id=source.chat_id,
            ...
            try:
                ...
                if rotated:
                    ...
                    if partial and tail:
                        # Fork patch: in-place compaction archived + inserted
                        # only the compressed HEAD; the verbatim tail rejoined
                        # above ("/compress here" keeps recent exchanges) is not
                        ...
                        # non-destructive path so the tail lands in the active
                        # set without deleting archived pre-compression history.
                    ...
                    # Fork patch: distinguish a compressor ABORT from a genuine
                    # rotation save failure. On abort the transcript is intact —
                    # fall through so the summary path surfaces the real aux
                    # model error instead of the misleading no-op message.
                    _aborted = bool(
                        getattr(
                            getattr(tmp_agent, "context_compressor", None),
