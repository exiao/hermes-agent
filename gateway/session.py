...
def _is_session_key_unsafe(value: object) -> bool:
    """Return True if ``value`` is unsafe for a logical session_key.

    ``session_key`` never becomes a filesystem path. Interior ``/`` is valid for
    platform IDs such as Signal base64 groups and Google Chat ``spaces/<id>``.
    Block only genuine traversal/log/DB hazards: parent-dir ``..``, NUL bytes, a
    leading path separator, and a leading Windows drive letter.
    """
    if not value:
        return False
    s = str(value)
...
class SessionEntry:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionEntry":
        ...
        # ``session_id`` is path-bound and must use the strict guard.
        # ``session_key`` is a logical routing key: interior ``/`` is legitimate
        # for platform IDs, so it uses the relaxed guard above.
