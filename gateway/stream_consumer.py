...
class StreamConsumerConfig:
    ...
    # Platforms without an edit API buffer short responses and split only at natural boundaries.
    no_edit_mode: bool = False
...
class GatewayStreamConsumer(StreamTransportMixin, StreamFallbackMixin, StreamThinkFilterMixin):
    ...
    def __init__(
        ...
        self._no_edit_mode = self.cfg.no_edit_mode
            ...
            while True:
                ...
                if self._should_edit(tick) and (
                    ...
                    if (
                        ...
                        last_break = self._accumulated.rfind("\n\n")
                        to_send = self._accumulated[:last_break]
                        self._accumulated = self._accumulated[last_break + 2:]
                        if to_send.strip():
                            await self._send_new_chunk(to_send, self._message_id)
    ...
    def _should_edit(self, tick: "_Tick") -> bool:
        ...
        if self._no_edit_mode:
            should_edit = (
                ("\n\n" in self._accumulated and len(self._accumulated) >= self._no_edit_min_buffer)
                or self._overflows()
            )
