...
class GatewayShutdownMixin:
    ...
    async def _cleanup_agent_resources_off_loop(
        self, agent: Any, *, context: str = "", session_messages: Any = _UNSET
    ) -> None:
        """Run resource cleanup off-loop, optionally using a pre-eviction transcript snapshot."""
        if agent is None:
            return
        ...
        try:
            await asyncio.wait_for(
                self._run_in_executor_with_context(
                    lambda: self._cleanup_agent_resources(
                        agent, session_messages=session_messages)),
    ...
    def _cleanup_agent_resources(
        self, agent: Any, *, session_messages: Any = _UNSET
    ) -> None:
