...
def test_skip_memory_still_loads_store_when_memory_tool_enabled():
    ...
    with (
        ...
        # Upstream's config-loader refactor made agent init read the config via
        # ``load_config_readonly`` (agent/agent_init.py), so patching only
        # ``load_config`` no longer intercepts it and the real ~/.hermes
        # config.yaml (memory_enabled: true) leaked in, flipping
        # ``_memory_enabled``. Patch the actual loader the code calls.
