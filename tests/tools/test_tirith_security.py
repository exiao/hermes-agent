...
class TestIsSafeLookalikeTldFinding:
    ...
    def test_bare_run_tld_suppressed(self):
        """.run is now a blanket-safe terminal TLD (operator request), like
        .app/.dev — a bare .run token is suppressed."""
        assert self.fn({"rule_id": "lookalike_tld", "value": ".run"})

    def test_run_message_field_suppressed(self):
        """A generic '.run' TLD warning is suppressed now that .run is safe."""
        assert self.fn({"rule_id": "lookalike_tld",
                        "message": "Domain uses '.run' TLD"})

    def test_arbitrary_run_host_suppressed(self):
        """Any terminal .run host is suppressed. This is the accepted tradeoff
        of treating .run as safe: an arbitrary attacker.run no longer warns."""
        assert self.fn({"rule_id": "lookalike_tld", "value": "attacker.run"})
    ...
    def test_evilmodal_run_suppressed(self):
        """Even a non-Modal terminal .run host is suppressed now (.run is a
        blanket-safe TLD, no longer Modal-specific)."""
        assert self.fn({"rule_id": "lookalike_tld", "value": "evilmodal.run"})
    ...
    def test_real_tirith_schema_run_evidence_suppressed(self):
        """Real Tirith schema-v3 output: the host lives in evidence[].raw and
        the description is generic. A terminal .run host is suppressed."""
        finding = {
    ...
    def test_real_tirith_schema_arbitrary_run_evidence_suppressed(self):
        """Same schema, arbitrary .run host in evidence — also suppressed now."""
        finding = {
            "rule_id": "lookalike_tld",
            "severity": "MEDIUM",
            "description": "Domain uses '.run' TLD which can be confused with file extensions",
    ...
    def test_evidence_non_dict_items_tolerated(self):
        ...
        # A terminal .run string is now suppressed too.
        assert self.fn({"rule_id": "lookalike_tld", "evidence": ["attacker.run"]})
