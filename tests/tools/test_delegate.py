...
class TestChildCredentialPoolResolution(unittest.TestCase):
    ...
    def test_kanban_worker_delegate_child_cannot_receive_lifecycle_tools(self, mock_cfg):
        ...
        # kanban must be denied to the review child so it cannot terminal-complete
        # its parent's task. Leaf children also inherit the role-blocked deny set
        # (delegation, clarify, memory, ...), so assert membership, not equality.
