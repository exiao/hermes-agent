...
def client(tmp_path, monkeypatch):
    ...
    # Route sequencing should not depend on the CI host's RAM or GPU capacity.
    budget = HardwareBudget(usable_vram_bytes=48 << 30, total_device_bytes=64 << 30,
                            ram_available_bytes=0, uma=True)
    monkeypatch.setattr("hermes_cli.local_runtime.hardware.probe_budget", lambda **_: budget)
