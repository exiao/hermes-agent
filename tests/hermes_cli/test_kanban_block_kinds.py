...
def test_ensure_reason_header_prepends_per_kind() -> None:
    ...
    assert kb.ensure_reason_header("429 from API", "transient") == (
        "RETRY: 429 from API"
    )


def test_ensure_reason_header_is_idempotent() -> None:
    once = kb.ensure_reason_header("which key?", "needs_input")
...
def test_ensure_reason_header_noop_for_dependency_and_none() -> None:
    assert kb.ensure_reason_header("wait for X", "dependency") == "wait for X"
    assert kb.ensure_reason_header("legacy", None) == "legacy"


def test_block_stamps_header_into_stored_reason_and_event(kanban_home: Path) -> None:
    """A typed block's stored reason + event payload lead with the header tag."""
    with kb.connect_closing() as conn:
        tid = _running_task(conn)
        kb.block_task(conn, tid, reason="which region first?", kind="needs_input")
        blocked_events = [
...
def test_block_capability_event_reads_routing(kanban_home: Path) -> None:
    with kb.connect_closing() as conn:
        tid = _running_task(conn)
        kb.block_task(conn, tid, reason="no prod creds", kind="capability")
        blocked_events = [
            e for e in kb.list_events(conn, tid) if e.kind == "blocked"
        ]
        payload = blocked_events[-1].payload or {}
        assert payload.get("reason", "").startswith("ROUTING:")


def test_untyped_block_reason_not_headed(kanban_home: Path) -> None:
    with kb.connect_closing() as conn:
        tid = _running_task(conn)
        ...
        payload = blocked_events[-1].payload or {}
        assert payload.get("reason") == "generic stuck"
