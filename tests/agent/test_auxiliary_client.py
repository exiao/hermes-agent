...
class TestResolveProviderClientAutoModelPollution:
    ...
    self-fallback config-default model override what ``_resolve_auto_route``
    resolved.
    ...
    with ``_read_main_model()`` = "claude-opus-4-8". ``_resolve_auto_route``
    ...
    def test_auto_explicit_caller_model_is_still_honored(self):
        ...
        with (
            ...
            patch(
                "agent.auxiliary_client._resolve_auto_route",
                return_value=(some_client, "gpt-5.5", "openai-codex"),
            ),
        ):
            client, model = resolve_provider_client(
                "auto",
    ...
    def test_auto_anthropic_session_still_resolves_claude(self):
        ...
        with (
            ...
            patch(
                "agent.auxiliary_client._resolve_auto_route",
                return_value=(anthropic_client, "claude-opus-4-8", "openai-codex"),
            ),
        ):
            client, model = resolve_provider_client(
                "auto",
