...
class TestCodexAuxiliaryTimeoutFdOwnership:
    def test_stalled_stream_timeout_shuts_down_from_timer_and_closes_from_owner(self):
        ...
        def _stalled():
            # Keep the owner inside the SDK iterator so the watchdog, rather
            # than the owner's next per-event check, owns the timeout race.
            yield SimpleNamespace(type="response.in_progress")
            assert shutdown_event.wait(timeout=5.0)
            yield SimpleNamespace(type="response.in_progress")

        adapter, events = _adapter_with_recording_client(
            _stalled(), shutdown_event=shutdown_event
        )
