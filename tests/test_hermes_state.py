...
class TestSessionLifecycle:
    ...
    def test_base_fts_rebuilds_when_one_base_trigger_is_missing(self, tmp_path):
        """Opening v21 repairs and backfills a partially missing FTS trigger set."""
        db_path = tmp_path / "state.db"
        seeded = SessionDB(db_path=db_path)
        try:
            seeded.create_session(session_id="s1", source="cli")
            seeded.append_message("s1", role="user", content="already indexed")
            # The three trigram triggers and two remaining base triggers stay
            # intact, so repair must inspect the complete trigger set.
