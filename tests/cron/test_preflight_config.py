...
class TestDeliveryPlatform:
    ...
    def test_live_adapter_allows_disabled_profile_target(self, tmp_path, monkeypatch):
        ...
        def fake_deliver(job, content, adapters=None, loop=None, *, for_failure=False):
            delivered.append((content, adapters, loop, for_failure))
            return None
        ...
        assert delivered == [("ok", {Platform.SIGNAL: adapter}, loop, False)]
